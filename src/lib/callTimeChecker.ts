const AREA_CODE_TZ: Record<string, string> = {
  ...
  '231':'America/New_York','234':'America/New_York',
  '239':'America/New_York','240':'America/New_York',
  '248':'America/New_York','251':'America/Chicago',
  '252':'America/New_York','253':'America/Los_Angeles',
  '254':'America/Chicago','256':'America/Chicago',
  '260':'America/New_York','262':'America/Chicago',
  '267':'America/New_York','269':'America/New_York',
  '270':'America/New_York','272':'America/New_York',
  ...
  '334':'America/Chicago','336':'America/New_York',
  '337':'America/Chicago','339':'America/New_York',
  ...
  '443':'America/New_York','469':'America/Chicago',
  '470':'America/New_York','475':'America/New_York',
  '478':'America/New_York','479':'America/Chicago',
  ...
  '689':'America/New_York','701':'America/Chicago',
  '702':'America/Los_Angeles','703':'America/New_York',
  '704':'America/New_York','706':'America/New_York',
  '707':'America/Los_Angeles','708':'America/Chicago',
  '712':'America/Chicago','713':'America/Chicago',
  ...
  '725':'America/Los_Angeles','726':'America/Chicago',
  ...
  '832':'America/Chicago','843':'America/New_York',
  '845':'America/New_York','847':'America/Chicago',
  '848':'America/New_York','850':'America/New_York',
  ...
  '865':'America/New_York','870':'America/Chicago',
  '872':'America/Chicago','878':'America/New_York',
  ...
  '973':'America/New_York','975':'America/Chicago',
  '978':'America/New_York','979':'America/Chicago',
...
export function getTimezoneForPhone(phone: string): string {
  ...
  // Extract area code: skip leading '1' country code if present
...
export function isWithinCallingHours(phone: string): {
  allowed: boolean
  timezone: string
  localTime: string
  nextWindow?: string
} {
  const timezone = getTimezoneForPhone(phone)
  ...
  // TCPA calling hours: 8:00 AM to 9:00 PM local time
  ...
  if (!allowed) {
    ...
    if (hour >= 21) {
      ...
      const dateParts = dateFormatter.formatToParts(tomorrow)
      const month = dateParts.find(p => p.type === 'month')?.value
      const day = dateParts.find(p => p.type === 'day')?.value
      const year = dateParts.find(p => p.type === 'year')?.value
      result.nextWindow = `${year}-${month}-${day} 08:00 ${timezone}`
    } else {
