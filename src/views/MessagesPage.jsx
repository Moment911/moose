...
const TYPE_COLORS = { client: '#E6007E', internal: '#3b82f6', project: '#22c55e' }
    ...
    if (mView==='thread' && selectedConvo) {
      return (
        <MobilePage padded={false}>
          <div style={{background: '#ffffff',padding:'14px 16px',display:'flex',alignItems:'center',gap:10}}>
            <button onClick={()=>setMView('list')} style={{background:'none',border:'none',cursor:'pointer',color:'#E6007E',padding:'4px 0',display:'flex',alignItems:'center',gap:4,WebkitTapHighlightColor:'transparent'}}>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'12px 16px',display:'flex',flexDirection:'column',gap:8}}>
            {(messages||[]).map(m=>(
              <div key={m.id} style={{display:'flex',flexDirection:'column',alignItems:m.is_agent?'flex-end':'flex-start'}}>
                <div style={{maxWidth:'82%',background:m.is_agent?'#E6007E':'#fff',borderRadius:m.is_agent?'14px 14px 4px 14px':'14px 14px 14px 4px',padding:'10px 13px',border:m.is_agent?'none':'1px solid #ececea'}}>
          ...
          <div style={{padding:'10px 16px',background:'#fff',borderTop:'1px solid #ececea',display:'flex',gap:8}}>
            ...
            <button onClick={()=>sendMessage&&sendMessage()}
              style={{width:44,height:44,borderRadius:'50%',background:'#E6007E',border:'none',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0,WebkitTapHighlightColor:'transparent'}}>
          ...
          <MobileCard style={{margin:'12px 16px'}}>
            {(conversations||[]).filter(cv=>!search||cv.title?.toLowerCase().includes(search.toLowerCase())||cv.client_name?.toLowerCase().includes(search.toLowerCase())).map((cv,i,arr)=>(
              <MobileRow key={cv.id}
                ...
                left={<div style={{width:42,height:42,borderRadius:'50%',background:'#E6007E'+'20',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Proxima Nova','Nunito Sans',sans-serif",fontSize:16,fontWeight:800,color:'#E6007E'}}>
                ...
                badge={cv.unread_count>0
                  ? <span style={{width:20,height:20,borderRadius:'50%',background:'#E6007E',color:'#fff',fontSize:11,fontWeight:800,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Proxima Nova','Nunito Sans',sans-serif",flexShrink:0}}>{cv.unread_count}</span>
                  : null}/>
            ))}
          </MobileCard>
        )}
