...
const COLORS = ['#3b82f6', '#22c55e', '#f59e0b', '#cb1c6b', '#8b5cf6', '#06b6d4', '#ec4899', '#231f20']
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white">
        ...
        <div className="h-14 bg-white border-b border-gray-200 px-6 flex items-center gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 ml-4">
            ...
            <button onClick={() => setCurrentDate(v => view === 'month' ? addMonths(v, 1) : addDays(v, 7))} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-700"><ChevronRight size={16} /></button>
          </div>
          ...
          <div className="flex bg-gray-100 rounded-lg p-0.5 ml-2">
            {['month', 'week'].map(v => (
              <button key={v} onClick={() => setView(v)} className={`text-sm px-3 py-1.5 rounded-md font-medium capitalize ${view === v ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-700'}`}>{v}</button>
            ))}
          </div>
          ...
          {view === 'month' && (
            <>
              <div className="grid grid-cols-7 border-b border-gray-200">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => <div key={d} className="py-2 text-center text-sm font-semibold text-gray-700 uppercase">{d}</div>)}
              </div>
                  ...
                  return (
                    <div key={i} className={`border-b border-r border-gray-100 p-1.5 min-h-[100px] cursor-pointer hover:bg-gray-50 transition-colors ${thisMonth ? 'bg-white' : 'bg-gray-50/50'} ${today ? 'ring-1 ring-inset ring-brand-200' : ''}`}
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <span className={`text-sm font-medium w-6 h-6 flex items-center justify-center rounded-full ${today ? 'bg-brand-500 text-white' : thisMonth ? 'text-gray-900' : 'text-gray-700'}`}>{format(day, 'd')}</span>
                      </div>
          ...
          {view === 'week' && (
            <div className="flex flex-1">
              ...
              <div className="w-16 flex-shrink-0 border-r border-gray-200 pt-10">
                {hours.map(h => <div key={h} className="h-12 text-[13px] text-gray-700 text-right pr-2 -mt-2">{h === 0 ? '' : `${h % 12 || 12}${h < 12 ? 'am' : 'pm'}`}</div>)}
              </div>
              {/* Day columns */}
        ...
        {/* Status bar */}
        <div className="h-8 bg-white border-t border-gray-100 px-4 flex items-center gap-4 text-sm text-gray-700 flex-shrink-0">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setShowModal(false)} className="text-gray-700 hover:text-gray-600"><X size={18} /></button>
            </div>
            <div className="px-5 py-4 space-y-3">
              ...
              <div className="flex gap-2">
                {EVENT_TYPES.map(t => (
                  <button key={t.key} onClick={() => setForm(f => ({ ...f, type: t.key, color: t.color }))}
                    className={`flex-1 text-sm py-2 rounded-lg font-medium border transition-colors ${form.type === t.key ? 'border-brand-400 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-700'}`}>
                    {t.label}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-3">
                <div className="flex-1"><label className="text-sm text-gray-700 mb-1 block">Start</label><input className="input text-sm" type="datetime-local" value={form.start_at} onChange={e => setForm(f => ({ ...f, start_at: e.target.value }))} /></div>
                <div className="flex-1"><label className="text-sm text-gray-700 mb-1 block">End</label><input className="input text-sm" type="datetime-local" value={form.end_at} onChange={e => setForm(f => ({ ...f, end_at: e.target.value }))} /></div>
              </div>
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="text-sm text-gray-700 mb-1 block">Reminder</label>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowModal(false)} className="text-sm text-gray-700 px-3 py-1.5">Cancel</button>
