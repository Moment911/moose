        ...
        <div style={{ padding:'0 18px 18px' }}>
          <div style={{ borderTop:'1px solid #f3f4f6', paddingTop:14 }}>
            <div style={{ background:'#f0f9ff', border:'1px solid #bae6fd', borderRadius:10, padding:'11px 14px', marginBottom:14 }}>
              <div style={{ fontSize:14, fontWeight:700, color:'#0369a1', marginBottom:7 }}>📋 {guide.title}</div>
              <ol style={{ margin:0, paddingLeft:18 }}>{guide.steps.map((s,i)=><li key={i} style={{ fontSize:15, color:'#374151', marginBottom:4, lineHeight:1.5 }}>{s}</li>)}</ol>
            ...
            {item.type==='credentials'&&(
              <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:12 }}>
                <div><label style={{ fontSize:14, fontWeight:700, color:'#374151', display:'block', marginBottom:5 }}>Login URL</label><input value={d.login_url||''} onChange={e=>set('login_url',e.target.value)} placeholder="https://admin.example.com" style={INP}/></div>
                <div><label style={{ fontSize:14, fontWeight:700, color:'#374151', display:'block', marginBottom:5 }}>Username or Email</label><input value={d.username||''} onChange={e=>set('username',e.target.value)} placeholder="admin@yourbusiness.com" style={INP}/></div>
                <div><label style={{ fontSize:14, fontWeight:700, color:'#374151', display:'block', marginBottom:5 }}>Password</label><PwField value={d.password} onChange={v=>set('password',v)}/></div>
                <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:7, padding:'7px 11px', fontSize:13, color:'#166534' }}>🔒 Encrypted — only your agency team can see this</div>
              </div>
            )}
            {item.type==='invite'&&<div style={{ marginBottom:12 }}><label style={{ fontSize:14, fontWeight:700, color:'#374151', display:'block', marginBottom:5 }}>Account URL or ID (optional)</label><input value={d.account_id||''} onChange={e=>set('account_id',e.target.value)} placeholder="e.g. facebook.com/yourpage" style={INP}/></div>}
            {item.type==='file_link'&&(
              <div style={{ marginBottom:12 }}>
                <label style={{ fontSize:14, fontWeight:700, color:'#374151', display:'block', marginBottom:5 }}>Document or Folder Link</label>
    ...
    <div style={{ minHeight:'100vh', background:'#f4f4f5' }}>
      ...
      <div style={{ maxWidth:720, margin:'0 auto', padding:'24px 18px 60px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', marginTop:20 }}>
          <button disabled={activeSection===0} onClick={()=>setActiveSection(s=>s-1)} style={{ display:'flex', alignItems:'center', gap:6, padding:'11px 20px', borderRadius:10, border:'1.5px solid #e5e7eb', background:'#fff', fontSize:15, cursor:activeSection===0?'not-allowed':'pointer', color:'#374151', opacity:activeSection===0?.4:1 }}><ChevronLeft size={15}/> Previous</button>
          {activeSection<ACCESS_SECTIONS.length-1
            ? <button onClick={()=>setActiveSection(s=>s+1)} style={{ display:'flex', alignItems:'center', gap:6, padding:'11px 24px', borderRadius:10, border:'none', background:ACCENT, color:'#fff', fontSize:15, fontWeight:700, cursor:'pointer' }}>Next Section<ChevronRight size={15}/></button>
            : <button onClick={()=>toast.success('All sections reviewed! Your agency has been notified.')} style={{ display:'flex', alignItems:'center', gap:6, padding:'11px 24px', borderRadius:10, border:'none', background:'#22c55e', color:'#fff', fontSize:15, fontWeight:700, cursor:'pointer' }}><CheckCircle size={15}/> All Done!</button>
          }
        </div>
      </div>
