...
const ACCENT = '#E8551A'
const INP = { width:'100%', padding:'8px 11px', borderRadius:8, border:'1.5px solid #e5e7eb', fontSize:13, outline:'none', background:'#fff', boxSizing:'border-box', color:'#111' }
...
function PwField({ value, onChange }) {
  ...
  return (
    <div style={{ position:'relative' }}>
      ...
      <button onClick={()=>setShow(s=>!s)} style={{ position:'absolute', right:8, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'#9ca3af' }}>
  ...
  return (
    <div style={{ display:'flex', gap:10, padding:'10px 0', borderBottom:'1px solid #f9fafb' }}>
      ...
      <div style={{ flex:1 }}>
        ...
        {entry.staff_note&&<div style={{ fontSize:11, color:'#6b7280', fontStyle:'italic' }}>"{entry.staff_note}"</div>}
        <div style={{ fontSize:10, color:'#9ca3af', marginTop:2 }}>{entry.created_at?formatDistanceToNow(new Date(entry.created_at),{addSuffix:true}):'—'}</div>
      </div>
    ...
    <div style={{ marginTop:10, padding:'10px 12px', background:'#f0fdf4', borderRadius:9, border:'1px solid #bbf7d0' }}>
      {latest ? (
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontSize:12, fontWeight:700, color:'#16a34a' }}>✓ Verified by {latest.changed_by}</div>
            <div style={{ fontSize:10, color:'#9ca3af' }}>{latest.created_at?format(new Date(latest.created_at),'MMM d, yyyy h:mm a'):''}{latest.staff_note?` · "${latest.staff_note}"`:''}</div>
          </div>
          <button onClick={()=>setShowForm(v=>!v)} style={{ fontSize:10, padding:'2px 7px', borderRadius:6, border:'1px solid #bbf7d0', background:'#fff', cursor:'pointer', color:'#16a34a' }}>Re-verify</button>
        </div>
      ) : (
        <button onClick={()=>setShowForm(v=>!v)} style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'none', cursor:'pointer', fontSize:12, color:'#16a34a', fontWeight:600, padding:0, width:'100%' }}>
          <Stamp size={13}/> Staff: Mark as Verified & Working
        </button>
      )}
      {showForm&&(
        <div style={{ marginTop:8, display:'flex', gap:7 }}>
          <input value={note} onChange={e=>setNote(e.target.value)} onKeyDown={e=>e.key==='Enter'&&verify()} placeholder="Optional note (e.g. 'Confirmed — full admin access')" style={{ ...INP, flex:1, fontSize:12 }}/>
          <button onClick={verify} disabled={saving} style={{ padding:'6px 13px', borderRadius:8, border:'none', background:'#16a34a', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', flexShrink:0 }}>{saving?'…':'Stamp ✓'}</button>
        </div>
      )}
    </div>
  )
    ...
    <div style={{ borderBottom:'1px solid #f3f4f6', background:isVerified?'#f9fffb':status==='complete'?'#fefffe':'#fff' }}>
      <div style={{ display:'grid', gridTemplateColumns:'26px 1fr 110px 90px 120px 110px 28px', gap:0, padding:'10px 14px', alignItems:'center', cursor:'pointer' }} onClick={onToggle}>
        ...
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <span style={{ fontSize:13, fontWeight:600, color:status==='complete'?'#9ca3af':'#111', textDecoration:status==='complete'?'line-through':'' }}>{item.label}</span>
            <span style={{ fontSize:10, padding:'1px 5px', borderRadius:6, background:'#f3f4f6', color:'#6b7280' }}>{typeCfg.icon}</span>
            {isVerified&&<ShieldCheck size={12} color="#16a34a"/>}
            {d.client_completed_at&&<span style={{ fontSize:9, background:'#eff6ff', color:'#1d4ed8', padding:'1px 5px', borderRadius:6, fontWeight:600 }}>CLIENT ✓</span>}
          </div>
          {clientAct.length>0&&<div style={{ fontSize:10, color:'#9ca3af' }}>Client updated {formatDistanceToNow(new Date(clientAct[0].created_at),{addSuffix:true})}</div>}
        </div>
        <div style={{ fontSize:11, color:'#9ca3af' }}>{item.access_level}</div>
        <div><span style={{ fontSize:10, fontWeight:700, padding:'2px 6px', borderRadius:8, background:priority.bg, color:priority.color, border:`1px solid ${priority.border}` }}>{item.priority.toUpperCase()}</span></div>
        <div onClick={e=>e.stopPropagation()}>
          <select value={status} onChange={e=>set('status',e.target.value)} style={{ ...INP, fontSize:11, padding:'3px 6px', background:statusCfg.bg, color:statusCfg.color, fontWeight:600, border:'none', cursor:'pointer' }}>
            {Object.entries(STATUS_CONFIG).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
          </select>
        </div>
        <div style={{ fontSize:11, color:isVerified?'#16a34a':'#d1d5db' }}>
      ...
      {expanded&&(
        <div style={{ padding:'0 14px 14px 40px', background:'#fafafa', borderTop:'1px solid #f3f4f6' }}>
          <div style={{ background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:9, padding:'10px 14px', marginBottom:12, marginTop:12, fontSize:12, color:'#1e40af', lineHeight:1.5 }}>
          ...
          {agencyEmail&&item.type==='invite'&&(
            <div style={{ background:'#fff7f5', border:`1px solid ${ACCENT}30`, borderRadius:9, padding:'8px 12px', marginBottom:10, fontSize:12, color:'#92400e', display:'flex', gap:8 }}>
          ...
          {(d.login_url||d.username||d.file_link||d.account_id||d.client_notes)&&(
            <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:9, padding:'10px 14px', marginBottom:10 }}>
              <div style={{ fontSize:10, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:7 }}>📥 Client Submitted</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                {[['Login URL',d.login_url,false],['Username',d.username,false],['Password',d.password,true],['File Link',d.file_link,false],['Account ID',d.account_id,false],['Client Notes',d.client_notes,false]].filter(([,v])=>v).map(([label,val,pw])=>(
                  <div key={label}>
                    <div style={{ fontSize:10, fontWeight:600, color:'#9ca3af', marginBottom:2, textTransform:'uppercase' }}>{label}</div>
                    {pw?<PwField value={val} onChange={v=>set('password',v)}/>:(
                      <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                        <span style={{ fontSize:12, color:'#111', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{val}</span>
                        <button onClick={()=>{navigator.clipboard.writeText(val);toast.success('Copied')}} style={{ background:'none', border:'none', cursor:'pointer', color:'#9ca3af', padding:1 }}><Copy size={10}/></button>
                      </div>
                    )}
                  </div>
                ))}
              </div>
              {d.client_completed_at&&<div style={{ fontSize:10, color:'#9ca3af', marginTop:6 }}>Completed: {format(new Date(d.client_completed_at),'MMM d, yyyy h:mm a')}</div>}
            </div>
          )}
          {/* Agency fields */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9, marginBottom:10 }}>
            {item.type==='credentials'&&!d.login_url&&<><div><label style={{ fontSize:10, fontWeight:700, color:'#9ca3af', display:'block', marginBottom:3, textTransform:'uppercase' }}>Login URL</label><input value={d.login_url||''} onChange={e=>set('login_url',e.target.value)} style={INP}/></div><div><label style={{ fontSize:10, fontWeight:700, color:'#9ca3af', display:'block', marginBottom:3, textTransform:'uppercase' }}>Username</label><input value={d.username||''} onChange={e=>set('username',e.target.value)} style={INP}/></div><div style={{ gridColumn:'1/-1' }}><label style={{ fontSize:10, fontWeight:700, color:'#9ca3af', display:'block', marginBottom:3, textTransform:'uppercase' }}>Password</label><PwField value={d.password||''} onChange={v=>set('password',v)}/></div></>}
            <div style={{ gridColumn:'1/-1' }}><label style={{ fontSize:10, fontWeight:700, color:'#9ca3af', display:'block', marginBottom:3, textTransform:'uppercase' }}>Internal Notes</label><input value={d.notes||''} onChange={e=>set('notes',e.target.value)} placeholder="Any caveats or notes…" style={INP}/></div>
          </div>
    ...
    <div style={{ display:'flex', minHeight:'100vh', background:'#f4f4f5' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0, overflow:'hidden' }}>
        ...
        <div style={{ background:'#fff', borderBottom:'1px solid #e5e7eb', padding:'11px 20px', display:'flex', alignItems:'center', gap:11, flexShrink:0 }}>
          <button onClick={()=>navigate('/clients')} style={{ background:'none', border:'none', cursor:'pointer', color:'#9ca3af' }}><ChevronLeft size={17}/></button>
          <div style={{ width:34, height:34, borderRadius:8, background:ACCENT, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:800, fontSize:14 }}>{client?.name?.[0]||'?'}</div>
          <div style={{ flex:1 }}><div style={{ fontSize:15, fontWeight:700, color:'#111' }}>{client?.name}</div><div style={{ fontSize:11, color:'#9ca3af' }}>Account Access Checklist · {complete}/{total} complete · {verified} verified</div></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 10px', borderRadius:20, background:isLive?'#f0fdf4':'#f3f4f6', border:isLive?'1px solid #bbf7d0':'1px solid #e5e7eb' }}>
            ...
            <span style={{ fontSize:10, fontWeight:700, color:isLive?'#16a34a':'#9ca3af' }}>{isLive?'Live':'Offline'}</span>
          </div>
          ...
          <div style={{ display:'flex', gap:7 }}>
            ...
            <button onClick={copyClientLink} style={{ display:'flex', alignItems:'center', gap:4, padding:'6px 12px', borderRadius:8, border:'1.5px solid #e5e7eb', background:'#fff', fontSize:12, cursor:'pointer', color:'#374151' }}><Link2 size={12}/> Client Link</button>
            <button onClick={()=>setShowActivity(s=>!s)} style={{ display:'flex', alignItems:'center', gap:4, padding:'6px 12px', borderRadius:8, border:`1.5px solid ${showActivity?ACCENT:'#e5e7eb'}`, background:showActivity?'#fff7f5':'#fff', fontSize:12, cursor:'pointer', color:showActivity?ACCENT:'#374151' }}><Activity size={12}/> Feed</button>
            <button onClick={saveAll} disabled={saving} style={{ display:'flex', alignItems:'center', gap:4, padding:'6px 14px', borderRadius:8, border:'none', background:ACCENT, color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer' }}><Save size={12}/>{saving?'Saving…':'Save'}</button>
          </div>
        </div>

        {showSettings&&<div style={{ background:'#fff7f5', borderBottom:'1px solid #fed7aa', padding:'9px 20px', display:'flex', alignItems:'center', gap:10 }}><span style={{ fontSize:12, fontWeight:600, color:'#92400e' }}>Agency Invite Email:</span><input value={agencyEmail} onChange={e=>setAgencyEmail(e.target.value)} style={{ padding:'5px 10px', borderRadius:7, border:'1.5px solid #fcd34d', fontSize:12, outline:'none', width:280, background:'#fff' }}/></div>}
        ...
        <div style={{ background:'#fff', borderBottom:'1px solid #e5e7eb', padding:'9px 20px', display:'flex', gap:4, alignItems:'center', flexShrink:0 }}>
          ...
          <div style={{ marginLeft:'auto', display:'flex', gap:7 }}>
            <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{ height:30, padding:'0 8px', borderRadius:8, border:'1.5px solid #e5e7eb', background:'#fff', fontSize:12, cursor:'pointer', outline:'none' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', display:'flex' }}>
          <div style={{ flex:1, overflowY:'auto', padding:'14px 20px' }}>
            {highPending>0&&<div style={{ background:'#fef2f2', border:'1px solid #fecaca', borderRadius:10, padding:'9px 14px', marginBottom:12, display:'flex', gap:9, alignItems:'center' }}><AlertTriangle size={14} color="#dc2626"/><span style={{ fontSize:13, fontWeight:700, color:'#991b1b', flex:1 }}>{highPending} high-priority items need client action</span></div>}
              ...
              return (
                <div key={section.id} style={{ background:'#fff', borderRadius:12, border:'1px solid #e5e7eb', overflow:'hidden', marginBottom:14 }}>
                  <div style={{ padding:'11px 14px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:10, background:`${section.color}06` }}>
                    ...
                    <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:700, color:'#111' }}>{section.label}</div><div style={{ fontSize:10, color:'#9ca3af' }}>{done}/{section.items.length} complete</div></div>
                    ...
                    <div style={{ fontSize:13, fontWeight:700, color:pct2===100?'#16a34a':section.color, minWidth:32, textAlign:'right' }}>{pct2}%</div>
                  </div>
                  <div style={{ display:'grid', gridTemplateColumns:'26px 1fr 110px 90px 120px 110px 28px', gap:0, padding:'5px 14px', background:'#f8f9fa', borderBottom:'1px solid #e5e7eb' }}>
                    {['','Platform','Access Level','Priority','Status','Verified By',''].map((h,i)=><div key={i} style={{ fontSize:9, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'.04em' }}>{h}</div>)}
                  </div>
          ...
          {showActivity&&(
            <div style={{ width:280, flexShrink:0, borderLeft:'1px solid #e5e7eb', background:'#fff', display:'flex', flexDirection:'column' }}>
              <div style={{ padding:'12px 14px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:7 }}>
                ...
                <span style={{ fontSize:12, fontWeight:700, color:'#111' }}>Live Activity</span>
                <span style={{ fontSize:10, color:'#9ca3af', marginLeft:'auto' }}>{liveActivity.length} events</span>
              </div>
              {liveActivity.length===0?(
                <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:8, color:'#9ca3af', padding:20, textAlign:'center' }}>
                  <Radio size={26} strokeWidth={1}/>
                  <div style={{ fontSize:12, fontWeight:600 }}>Waiting for activity</div>
                  <div style={{ fontSize:11 }}>Client actions appear here in real time</div>
                </div>
              ):(
