...
import { Search as SearchIcon, Code2, ShieldCheck, Edit3, Repeat, TrendingUp, Plug, Globe, Loader2, ExternalLink, Plus, X, User, PowerOff, ChevronLeft, ChevronRight, RefreshCw } from 'lucide-react'
...
import ElementorBuilderPanel  from '../components/kotoiq/ElementorBuilderPanel'
import ContentRotationPanel   from '../components/kotoiq/ContentRotationPanel'
import SEOPanel               from '../components/kotoiq/SEOPanel'

/**
 * KotoIQSitesPage — per-client landing page for KotoIQ-paired WordPress sites,
 * exposing all 6 modules as tabs. Separate from the SaaS-product KotoIQ
 * dashboard (KotoIQPage.jsx); this view is exclusively the WP-plugin control
 * surface that used to live at /wpsimplecode.
 *
 * Tabs:
 *   • Search & Replace  (S&R)
 *   • Snippets          (PHP/HTML/JS/CSS injection)
 *   • Access            (per-role cap matrix)
 *   • Builder           (Elementor read/write inventory)
 *   • Rotation          ([koto_rotate] cache lookup)
 *   • SEO               (Yoast/Rank Math, sitemap, page factory overview)
 *
 * Brand: navy (BLK) + pink (R) on cream (GRY) — Unified Marketing palette.
 */
...
function StatusPills({ site }) {
  ...
  return (
    <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
      ...
      {site.wpsc_api_key
        ? <Pill color={R} bg={`${R}15`}>{isKotoIQ ? 'KotoIQ paired' : 'WPSC paired'}</Pill>
...
export default function KotoIQSitesPage() {
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ padding:'14px 28px', borderBottom:'1px solid #e5e7eb', background:'#fff', display:'flex', alignItems:'center', gap:14 }}>
          <div style={{ width:34, height:34, borderRadius:9, background:`${R}12`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
            <SearchIcon size={18} color={R}/>
          </div>
          <div style={{ flex:1, display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
            <span style={{ fontFamily:FH, fontSize:15, fontWeight:800, color:BLK }}>KotoIQ Sites</span>
            {agencyName && <>
                  ...
                  {activeSite && (
                    <div style={{ display:'flex', gap:2, overflowX:'auto' }}>
                      {TABS.map(t => {
                        ...
                        return (
                          <button key={t.key} onClick={() => setTab(t.key)} title={enabled ? t.label : `${t.label} (module disabled on this site)`} style={{
                            display:'flex', alignItems:'center', gap:6,
                            padding:'9px 14px', borderRadius:'8px 8px 0 0', border:'none',
                            background: active ? GRY : 'transparent',
                            color: active ? BLK : (enabled ? '#9ca3af' : '#d1d5db'),
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(15,23,42,.45)', display:'flex', alignItems:'flex-start', justifyContent:'center', paddingTop:80, zIndex:1000 }}>
      <div style={{ background:'#fff', borderRadius:14, maxWidth:540, width:'100%', padding:22, boxShadow:'0 30px 80px rgba(0,0,0,.18)' }}>
        ...
        <div style={{ fontSize:12, color:'#6b7280', fontFamily:FB, marginBottom:16, lineHeight:1.5 }}>
          Install <strong>KotoIQ</strong> on the site, then copy its API key from <em>WP admin → KotoIQ → Settings</em>. The legacy Koto plugin key is optional and only needed for sites still running the older koto plugin alongside KotoIQ.
        </div>
