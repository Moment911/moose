...
function ScoreRing({ score, size = 80, color }) {
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: size * 0.3, fontWeight: 900, color: col }}>{s}</div>
    </div>
  )
}
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <h3 style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 17, fontWeight: 800, color: BLK, margin: 0 }}>{title}</h3>
      </div>
      {children}
    </div>
  )
...
function EmptyState({ message, tab, onSwitchTab }) {
  return (
    <div style={{ background: AMB + '10', border: `1px solid #ececef`, borderRadius: 10, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <span style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#92400e', flex: 1 }}>{message}</span>
      {tab && onSwitchTab && (
        <button onClick={() => onSwitchTab(tab)} style={{ display: 'flex', alignItems: 'center', gap: 4, background: "#0a0a0a", color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
...
function MetricPill({ label, value }) {
  return (
    <div style={{ background: '#f9f9fb', borderRadius: 10, padding: '10px 16px', textAlign: 'center', flex: '1 1 0', minWidth: 100 }}>
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 20, fontWeight: 900, color: BLK }}>{value ?? '—'}</div>
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 11, color: '#6b6b70', marginTop: 2 }}>{label}</div>
    </div>
  )
}
...
function AiInterpretation({ text }) {
  ...
  return (
    <div style={{ background: '#f9f9fb', border: `1px solid #ececef`, borderRadius: 10, padding: '12px 16px', marginTop: 14 }}>
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 11, fontWeight: 700, color: T, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}>AI Interpretation</div>
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#1f1f22', lineHeight: 1.55 }}>{text}</div>
    </div>
  )
}
...
export default function MasterReport({ clientId, agencyId, onSwitchTab }) {
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 12 }}>
      ...
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, color: '#6b6b70' }}>Loading master report...</div>
    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <SectionCard icon={BarChart2} title="Executive Summary">
        <div style={{ display: 'flex', alignItems: 'center', gap: 24, marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 700, color: '#6b6b70', marginBottom: 4 }}>Overall Health: {avgScore !== null ? `${grade(avgScore)} (${avgScore}/100)` : 'Insufficient data'}</div>
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#1f1f22', lineHeight: 1.6 }}>
          ...
          {(kw.categories || kw.buckets) && (
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {Object.entries(kw.categories || kw.buckets || {}).map(([k, v]) => (
                <span key={k} style={{ background: '#f1f1f6', borderRadius: 8, padding: '4px 12px', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#1f1f22' }}>
      ...
      <SectionCard icon={Award} title="Topical Authority">
        {data.authority ? (<>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 14 }}>
            ...
            <div>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 700, color: BLK }}>Authority Score: {auth.score ?? '—'}/100</div>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#6b6b70' }}>Grade: {auth.grade ?? grade(auth.score ?? 0)}</div>
            </div>
          </div>
          {auth.clusters && auth.clusters.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: '#6b6b70', marginBottom: 6 }}>Cluster Breakdown</div>
              {auth.clusters.slice(0, 8).map((cl, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #f1f1f6', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13 }}>
          ...
          {auth.gaps && auth.gaps.length > 0 && (
            <div style={{ background: '#fef3c7', borderRadius: 8, padding: '10px 14px', marginTop: 8 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: '#92400e', marginBottom: 4 }}>Gaps</div>
              {auth.gaps.slice(0, 5).map((g, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#78350f' }}>{typeof g === 'string' ? g : g.topic || g.name}</div>)}
            </div>
          )}
      ...
      <SectionCard icon={Target} title="Strategic Plan">
        {data.strategy ? (<>
          {strat.attack && strat.attack.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: GRN, marginBottom: 4 }}>Attack Priorities</div>
              {strat.attack.slice(0, 5).map((a, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#1f1f22', padding: '2px 0' }}>{typeof a === 'string' ? a : a.keyword || a.topic || JSON.stringify(a)}</div>)}
            </div>
          )}
          {strat.defend && strat.defend.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: AMB, marginBottom: 4 }}>Defend Priorities</div>
              {strat.defend.slice(0, 5).map((d, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#1f1f22', padding: '2px 0' }}>{typeof d === 'string' ? d : d.keyword || d.topic || JSON.stringify(d)}</div>)}
            </div>
          )}
          {strat.abandon && strat.abandon.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: '#8e8e93', marginBottom: 4 }}>Consider Abandoning</div>
              {strat.abandon.slice(0, 3).map((a, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#8e8e93', padding: '2px 0' }}>{typeof a === 'string' ? a : a.keyword || a.topic || JSON.stringify(a)}</div>)}
            </div>
          )}
          {strat.weekly_actions && <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#1f1f22', marginTop: 8 }}><strong>Weekly Actions:</strong> {Array.isArray(strat.weekly_actions) ? strat.weekly_actions.join(', ') : strat.weekly_actions}</div>}
      ...
      <SectionCard icon={Shield} title="E-E-A-T Analysis">
        {data.eeat ? (<>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 14 }}>
            ...
            <div>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 700, color: BLK }}>E-E-A-T Score: {eeat.overall_score ?? eeat.score ?? '—'}/100</div>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#6b6b70' }}>Grade: {eeat.grade ?? grade(eeat.overall_score ?? eeat.score ?? 0)}</div>
            </div>
          </div>
          {eeat.signals && eeat.signals.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              {eeat.signals.slice(0, 8).map((s, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13 }}>
          ...
          {eeat.recommendations && eeat.recommendations.length > 0 && (
            <div style={{ background: '#f0fdf4', borderRadius: 8, padding: '10px 14px', marginTop: 8 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: GRN, marginBottom: 4 }}>Recommendations</div>
              {eeat.recommendations.slice(0, 4).map((r, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#166534' }}>{typeof r === 'string' ? r : r.text || r.recommendation}</div>)}
            </div>
          )}
        ...
        {data.brandSerp ? (<>
          ...
          {(bs.negative_results ?? bs.negatives ?? []).length > 0 && (
            <div style={{ background: R + '10', borderRadius: 8, padding: '10px 14px', marginTop: 8 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: R, marginBottom: 4 }}>Negative Results</div>
              {(bs.negative_results ?? bs.negatives).slice(0, 3).map((n, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#991b1b' }}>{typeof n === 'string' ? n : n.title || n.url}</div>)}
            </div>
          )}
      ...
      <SectionCard icon={Code} title="Technical SEO">
        {data.technical ? (<>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 700, color: BLK }}>Technical Score: {tech.overall_score ?? tech.score ?? '—'}/100</div>
              {tech.sub_scores && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 6 }}>
                  {Object.entries(tech.sub_scores).map(([k, v]) => (
                    <span key={k} style={{ background: '#f1f1f6', borderRadius: 6, padding: '2px 10px', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 11, color: '#1f1f22' }}>{k}: <strong>{v}</strong></span>
                  ))}
                </div>
              )}
            </div>
          ...
          {(tech.critical_issues ?? tech.issues ?? []).length > 0 && (
            <div style={{ background: '#f9f9fb', borderRadius: 8, padding: '10px 14px' }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: R, marginBottom: 4 }}>Critical Issues</div>
              {(tech.critical_issues ?? tech.issues).slice(0, 5).map((issue, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#991b1b', padding: '2px 0' }}>{typeof issue === 'string' ? issue : issue.title || issue.description}</div>)}
            </div>
          )}
        ...
        {data.schema ? (<>
          ...
          {(sch.missing ?? sch.missing_opportunities ?? []).length > 0 && (
            <div style={{ background: AMB + '10', borderRadius: 8, padding: '10px 14px' }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, color: '#92400e', marginBottom: 4 }}>Missing Opportunities</div>
              {(sch.missing ?? sch.missing_opportunities).slice(0, 5).map((m, i) => <div key={i} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, color: '#78350f' }}>{typeof m === 'string' ? m : m.type || m.name}</div>)}
            </div>
          )}
      ...
      {gaps.length > 0 && (
        <SectionCard icon={AlertTriangle} title="Gaps & Missing Data">
          <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, color: '#6b6b70', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {gaps.map((g, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fffbeb', borderRadius: 8, padding: '10px 14px' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 700, color: BLK }}>{g.label}</div>
                  <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 11, color: '#6b6b70' }}>{g.msg}</div>
                </div>
                <button onClick={() => onSwitchTab?.(g.tab)} style={{ background: "#0a0a0a", color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                  Go <ArrowRight size={12} />
                </button>
              </div>
            ))}
