...
function StatusPill({ status }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: color + '18', color, textTransform: 'uppercase', letterSpacing: '.04em', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>
      <Icon size={10} /> {s}
    </span>
  )
}
    ...
    <div>
      <div style={card}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          ...
          <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 18, fontWeight: 800, color: BLK }}>Activity</div>
          <div style={{ marginLeft: 'auto', fontSize: 12, color: '#6b6b70', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>
            {activities.length} action{activities.length === 1 ? '' : 's'} on file
          </div>
        </div>
        <div style={{ fontSize: 13, color: '#1f1f22', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", marginBottom: 12 }}>
      ...
      {loading && (
        <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 8, color: '#6b6b70', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13 }}>
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ ...card, textAlign: 'center', padding: '36px 20px', color: '#6b6b70', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>
          <Clock size={28} color="#d1d5db" style={{ marginBottom: 8 }} />
          <div style={{ fontSize: 14, fontWeight: 600, color: BLK, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", marginBottom: 4 }}>
      ...
      {!loading && groups.map(g => (
        <div key={g.label} style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: '#6b6b70', textTransform: 'uppercase', letterSpacing: '.06em', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", padding: '0 4px 8px' }}>
            {g.label}
          </div>
          {g.items.map(a => {
            ...
            return (
              <div key={a.id} style={{ ...card, padding: '14px 16px', borderLeft: `3px solid ${a.status === 'reverted' ? '#8e8e93' : a.status === 'failed' ? '#e9695c' : T}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 800, color: BLK }}>
                    {prettyIntent(a.intent)}
                  </div>
                  <StatusPill status={a.status} />
                  <div style={{ marginLeft: 'auto', fontSize: 11, color: '#6b6b70', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", display: 'flex', flexDirection: 'column', alignItems: 'flex-end', lineHeight: 1.2 }}>
                ...
                {resultSummary && (
                  <div style={{ fontSize: 12, color: '#1f1f22', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", marginBottom: 10 }}>
                    {resultSummary}
                  </div>
                )}
                {a.status === 'reverted' && a.reverted_at && (
                  <div style={{ fontSize: 11, color: '#6b6b70', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", marginBottom: 10, fontStyle: 'italic' }}>
