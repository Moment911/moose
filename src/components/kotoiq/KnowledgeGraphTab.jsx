...
function renderMarkdown(text) {
  ...
  return paragraphs.map((para, pIdx) => {
    ...
    if (heading && lines.length === 1) {
      ...
      return <Tag key={pIdx} style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 16 - level, fontWeight: 800, color: BLK, margin: '12px 0 6px' }}>{heading[2]}</Tag>
    }
    const isList = lines.every(l => /^\s*[-*\d]/.test(l) || !l.trim())
    if (isList && lines.some(l => l.trim())) {
      return (
        <ol key={pIdx} style={{ margin: '6px 0 10px', paddingLeft: 22, fontSize: 13, lineHeight: 1.65, color: '#1f1f22', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>
          {lines.filter(l => l.trim()).map((l, i) => <li key={i}>{l.replace(/^\s*[-*\d]+\.?\s*/, '')}</li>)}
        </ol>
      )
    }
    return <p key={pIdx} style={{ margin: '6px 0', fontSize: 13, lineHeight: 1.65, color: '#1f1f22', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>{para}</p>
  })
}
  ...
  return (
    <div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 4 }}>Knowledge Graph Export</div>
      ...
      {/* Format toggle */}
      <div style={card}>
        <div style={{ fontSize: 11, fontWeight: 800, color: '#1f2937', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>Export Format</div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{
              ...
              background: "#0a0a0a", color: '#fff', fontSize: 13, fontWeight: 700, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif",
        ...
        <>
          ...
          <div style={card}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: '#1f2937', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>
              ...
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 42, fontWeight: 900, color: kpColor, letterSpacing: '-.02em' }}>{kpScore}<span style={{ fontSize: 18, color: '#8e8e93' }}>/100</span></div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr 1fr', gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ ...card, margin: 0 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 800, color: BLK, marginBottom: 12 }}>Entity Properties</div>
            ...
            <div style={{ ...card, margin: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 800, color: BLK }}>Generated {FORMATS.find(f => f.key === format)?.label}</div>
            ...
            {/* Related entities */}
            <div style={{ ...card, margin: 0 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 800, color: BLK, marginBottom: 12 }}>Related Entities</div>
              <div style={{ maxHeight: 500, overflowY: 'auto' }}>
