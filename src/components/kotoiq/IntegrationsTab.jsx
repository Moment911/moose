  ...
  return (
    <div style={{ ...card, marginBottom: 0, borderTop: `4px solid ${color}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 16, fontWeight: 800, color: BLK }}>{label}</div>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: '#1f1f22', marginBottom: 5, fontFamily: DESIGN.fonts.body, textTransform: 'uppercase', letterSpacing: '.05em' }}>Webhook URL</div>
        <input value={state.webhook_url} onChange={e => setState({ ...state, webhook_url: e.target.value })}
          ...
          style={{
            ...
            fontSize: 12, fontFamily: DESIGN.fonts.body, boxSizing: 'border-box',
          }} />
      </div>

      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: '#1f1f22', marginBottom: 8, fontFamily: DESIGN.fonts.body, textTransform: 'uppercase', letterSpacing: '.05em' }}>Alert Types</div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <button onClick={save} disabled={saving} style={{
          ...
          border: 'none', background: color, color: '#fff', fontSize: 12, fontWeight: 700, fontFamily: DESIGN.fonts.body,
        ...
        <button onClick={testMessage} disabled={testing || !state.connected} style={{
          display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', borderRadius: 8,
          border: '1px solid #ececef', background: '#fff', color: '#1f1f22', fontSize: 12, fontWeight: 700, fontFamily: DESIGN.fonts.body,
        ...
        {state.connected && (
          <button onClick={disconnect} style={{
            ...
            border: '1px solid #fecaca', background: '#fff', color: R, fontSize: 12, fontWeight: 700, fontFamily: DESIGN.fonts.body,
    ...
    <div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        ...
        <button onClick={sendDigest} disabled={sending} style={{
          ...
          border: 'none', background: "#0a0a0a", color: '#fff', fontSize: 14, fontWeight: 700, fontFamily: DESIGN.fonts.body,
      ...
      <div style={{ ...card, background: 'linear-gradient(135deg,#f0f9ff 0%,#fefce8 100%)', borderLeft: `4px solid ${T}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK }}>Automated Schedule</div>
        </div>
        <div style={{ fontSize: 13, color: '#1f1f22', lineHeight: 1.6, fontFamily: DESIGN.fonts.body }}>
      ...
      <div style={card}>
        <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 800, color: BLK, marginBottom: 12 }}>
          Alert History
        </div>
        {history.length === 0 ? (
