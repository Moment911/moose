...
import { FH, BLK, GRY, R, T, DESIGN } from '../../lib/theme'
...
export default function AttributionTab({ publishId: initialPublishId, siteId, agencyId }) {
  ...
  if (!publishId) {
    ...
    return (
      <div>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, fontFamily: DESIGN.fonts.body }}>
          Published Pages
        </div>
        {publishes.length === 0 ? (
  ...
  if (!kpis) {
    return (
      <div style={{ ...card, textAlign: 'center', padding: 40, color: '#6b6b70' }}>
        <Info size={20} style={{ marginBottom: 8 }} />
        <div style={{ fontSize: 14, fontFamily: DESIGN.fonts.body }}>No attribution data found for this publish.</div>
            ...
            {Object.entries(CWV_THRESHOLDS).map(([key, meta]) => {
              ...
              return (
                <div key={key} style={{ textAlign: 'center', padding: '12px 8px', borderRadius: 10, border: '1px solid #ececef' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, fontFamily: DESIGN.fonts.body, color: '#6b6b70', textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 6 }}>
                    {meta.label}
                  </div>
                  <div style={{ fontSize: 22, fontWeight: 800, fontFamily: DESIGN.fonts.body, color: GRADE_COLORS[grade] }}>
            ...
            {kpis.attributed_calls.map((call, i) => (
              <div key={call.inbound_call_id || i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 8, border: '1px solid #ececef', background: '#fafafb' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, fontFamily: DESIGN.fonts.body, color: BLK }}>
  ...
  return (
    <div style={{ ...card, padding: '16px 18px', marginBottom: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, fontFamily: DESIGN.fonts.body, color: '#6b6b70', textTransform: 'uppercase', letterSpacing: '.04em' }}>
          {label}
        </span>
      </div>
      <div style={{ fontSize: 24, fontWeight: 800, fontFamily: DESIGN.fonts.body, color: BLK }}>
        {value}
      </div>
      {sub && (
