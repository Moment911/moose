...
function ScoreBadge({ score, label, size = 'md' }) {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize, fontWeight: 900, color, lineHeight: 1 }}>{score ?? '—'}</div>
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 18, fontWeight: 800, color: BLK }}>Review & Reputation Intelligence</div>
        ...
        <div style={{ ...card, textAlign: 'center', padding: '48px 24px' }}>
          ...
          <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 16, fontWeight: 700, color: BLK, marginBottom: 8 }}>No review analysis yet</div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '28px 36px', minWidth: 140 }}>
          <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 48, fontWeight: 900, color: (data?.overall_score || 0) >= 70 ? GRN : (data?.overall_score || 0) >= 40 ? AMB : R, lineHeight: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          <div style={card}>
            ...
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 28, fontWeight: 900, color: BLK }}>{data?.total_reviews || 0}</div>
          </div>
          <div style={card}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#1f2937', textTransform: 'uppercase', marginBottom: 4 }}>Velocity</div>
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 28, fontWeight: 900, color: BLK }}>{data?.review_velocity || 0}</div>
          ...
          <div style={card}>
            ...
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 28, fontWeight: 900, color: (data?.response_rate || 0) >= 80 ? GRN : (data?.response_rate || 0) >= 50 ? AMB : R }}>{data?.response_rate || 0}%</div>
          </div>
          <div style={card}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#1f2937', textTransform: 'uppercase', marginBottom: 4 }}>Avg Response</div>
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 28, fontWeight: 900, color: BLK }}>{data?.avg_response_time_hours ? `${Math.round(data.avg_response_time_hours)}h` : '—'}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...card, marginBottom: 20 }}>
        <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 800, color: BLK, marginBottom: 12 }}>Rating Distribution</div>
      ...
      {sentimentTopics.length > 0 && (
        <div style={{ ...card, marginBottom: 20 }}>
          <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 800, color: BLK, marginBottom: 12 }}>Sentiment by Topic</div>
            ...
            {sentimentTopics.map((topic, i) => (
              <div key={i} style={{ padding: '14px 16px', borderRadius: 10, border: '1px solid #ececef', cursor: 'pointer' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 700, color: BLK }}>{topic.topic}</div>
      ...
      {unresponded.length > 0 && (
        <div style={{ ...card, marginBottom: 20, borderColor: '#fecaca' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 800, color: R }}>Unresponded Negative Reviews ({unresponded.length})</div>
          </div>
      ...
      <div style={{ ...card }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 14, fontWeight: 800, color: BLK }}>Review Campaigns</div>
          ...
          <div style={{ padding: '16px 20px', borderRadius: 10, border: '1px solid #ececef', marginBottom: 16, background: GRY }}>
            <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
              <input value={campaignName} onChange={e => setCampaignName(e.target.value)}
                ...
                style={{ flex: 1, padding: '10px 14px', borderRadius: 8, border: '1px solid #ececef', fontSize: 13, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }} />
              <input type="number" value={campaignTarget} onChange={e => setCampaignTarget(parseInt(e.target.value) || 10)}
                min={1} max={100} style={{ width: 80, padding: '10px 14px', borderRadius: 8, border: '1px solid #ececef', fontSize: 13, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", textAlign: 'center' }} />
        ...
        {campaigns.map(camp => {
          ...
          return (
            <div key={camp.id} style={{ padding: '14px 16px', borderRadius: 10, border: '1px solid #ececef', marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 700, color: BLK }}>{camp.name}</div>
