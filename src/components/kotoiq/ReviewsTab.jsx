      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>Analyze your reviews, track sentiment, and build review campaigns.</div>
          </div>
        </div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '28px 36px', minWidth: 140 }}>
          ...
          <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 4, textTransform: 'uppercase', fontWeight: 700, letterSpacing: '.05em' }}>Review Score</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', marginBottom: 4 }}>Total Reviews</div>
            <div style={{ fontFamily: FH, fontSize: 28, fontWeight: 900, color: BLK }}>{data?.total_reviews || 0}</div>
          </div>
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', marginBottom: 4 }}>Velocity</div>
          ...
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', marginBottom: 4 }}>Response Rate</div>
          ...
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', marginBottom: 4 }}>Avg Response</div>
          ...
          {[5, 4, 3, 2, 1].map(star => {
            ...
            return (
              <div key={star} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ width: 32, fontSize: 12, fontWeight: 700, color: '#6b7280', textAlign: 'right' }}>{count}</div>
              </div>
            )
          })}
        </div>
            ...
            {sentimentTopics.map((topic, i) => (
              <div key={i} style={{ padding: '14px 16px', borderRadius: 10, border: '1px solid #e5e7eb', cursor: 'pointer' }}
                ...
                <div style={{ display: 'flex', gap: 12, fontSize: 10, fontWeight: 600 }}>
                  ...
                  <span style={{ color: '#9ca3af' }}>{topic.neutral_pct || 0}% neutral</span>
                ...
                {expandedTopic === i && topic.sample_quotes?.length > 0 && (
                  <div style={{ marginTop: 10, borderTop: '1px solid #f3f4f6', paddingTop: 10 }}>
                    {topic.sample_quotes.map((q, qi) => (
                      <div key={qi} style={{ fontSize: 12, color: '#6b7280', fontStyle: 'italic', marginBottom: 4, lineHeight: 1.5 }}>
                        "{q}"
                      </div>
                    ))}
                  </div>
            ...
            {unresponded.slice(0, 10).map((review, i) => (
              <div key={i} style={{ padding: '12px 16px', borderRadius: 8, background: '#fef2f2', border: '1px solid #fecaca' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#9ca3af' }}>{review.date ? new Date(review.date).toLocaleDateString() : ''}</span>
                </div>
        ...
        {showCampaignForm && (
          <div style={{ padding: '16px 20px', borderRadius: 10, border: '1px solid #e5e7eb', marginBottom: 16, background: GRY }}>
            <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 12, color: '#6b7280', alignSelf: 'center' }}>target</span>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div key={camp.id} style={{ padding: '14px 16px', borderRadius: 10, border: '1px solid #e5e7eb', marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 12, background: statusColor + '18', color: statusColor, textTransform: 'uppercase' }}>
                    {camp.status}
                  </span>
                </div>
                <div style={{ fontSize: 11, color: '#9ca3af' }}>{new Date(camp.created_at).toLocaleDateString()}</div>
              </div>
              {/* Progress bar */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
