...
const input = { width: '100%', padding: '10px 12px', border: '1px solid #ececef', borderRadius: 8, fontSize: 13, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", boxSizing: 'border-box' }
const label = { fontSize: 12, fontWeight: 700, color: '#1f1f22', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", marginBottom: 4, display: 'block' }
...
export default function AdsAdBuilderTab({ clientId, agencyId }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 16 }}>Ad Copy Builder</div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        {PLATFORMS.map(p => (
          <button key={p.id} onClick={() => { setPlatform(p.id); setResult(null) }}
            ...
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontWeight: 800, fontSize: 14, color: platform === p.id ? '#5aa0ff' : BLK }}>{p.name}</div>
      ...
      {/* Brief Form */}
      <div style={card}>
        <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 15, fontWeight: 800, color: BLK, marginBottom: 14 }}>Creative Brief</div>
        ...
        <button onClick={generate} disabled={generating || !brief.offer}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 20px', background: "#0a0a0a", color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", cursor: 'pointer', opacity: generating || !brief.offer ? 0.6 : 1 }}>
      ...
      {result && (
        <>
          ...
          {platform === 'google' && result.headlines && (
            <div style={card}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 15, fontWeight: 800, color: BLK, marginBottom: 12 }}>Google RSA — 15 Headlines + 4 Descriptions</div>
              ...
              {result.variants.map((v, i) => (
                <div key={i} style={card}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                    <span style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontWeight: 800, fontSize: 14, color: T }}>Variant {String.fromCharCode(65 + i)}</span>
