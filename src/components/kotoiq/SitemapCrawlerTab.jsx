  ...
  return (
    <div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 4 }}>Sitemap Crawler</div>
          <div style={{ fontSize: 13, color: '#1f1f22' }}>Discover every URL on your site — handles multi-sitemap indexes, 10,000+ URLs.</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 36, fontWeight: 900, color: T, letterSpacing: '-.02em' }}>{total.toLocaleString()}</div>
      ...
      {/* Options */}
      <div style={card}>
        <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK, marginBottom: 14 }}>Crawl Options</div>
        ...
        <div style={{ marginTop: 14, display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={startCrawl} disabled={running} style={{
            ...
            fontSize: 13, fontWeight: 700, fontFamily: DESIGN.fonts.body, cursor: running ? 'wait' : 'pointer',
      ...
      <div style={card}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 10, flexWrap: 'wrap' }}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK }}>URL Explorer</div>
      ...
      {urls.length > 0 && (
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK }}>Sitemap Recommendations</div>
          </div>
...
const input = { width: '100%', padding: '8px 10px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: 12, fontFamily: DESIGN.fonts.body, color: BLK, outline: 'none', background: '#fff' }
...
function Field({ label, children }) {
  return (
    <div>
      <div style={{ fontSize: 10, fontWeight: 800, color: '#6b6b70', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 4, fontFamily: DESIGN.fonts.body }}>{label}</div>
      {children}
    </div>
  )
}
...
function Stat({ label, value, color }) {
  return (
    <div style={{ background: '#f9f9fb', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
      <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 900, color: color || BLK }}>{typeof value === 'number' ? value.toLocaleString() : value}</div>
