...
function ScoreRing({ score, label, color, size = 64 }) {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ marginTop: -size / 2 - 10, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: size > 50 ? 18 : 14, fontWeight: 900, color, lineHeight: `${size}px` }}>{score}</div>
      {label && <div style={{ fontSize: 12, fontWeight: 700, color: '#1f2937', textTransform: 'uppercase', marginTop: 4, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>{label}</div>}
    </div>
  )
}
...
function StatBox({ label, value, color, sub }) {
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #ececef', padding: '16px 18px', textAlign: 'center' }}>
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 28, fontWeight: 900, color: color || BLK }}>{value}</div>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#1f2937', textTransform: 'uppercase', marginTop: 4, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>{label}</div>
  ...
  if (!audit) {
    return (
      <div style={{ ...card, textAlign: 'center', padding: '60px 24px' }}>
        ...
        <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 8 }}>Schema & Structured Data</div>
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 22, fontWeight: 900, color: BLK }}>Schema Markup Audit</div>
      </div>
      ...
      {hasNoData && (
        <div style={{ ...card, textAlign: 'center', padding: '32px 24px', background: '#fffbeb', borderColor: AMB + '30' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 700, color: BLK, marginBottom: 6, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>No schema data yet</div>
            ...
            {Object.entries(schemaTypes).sort((a, b) => Number(b[1]) - Number(a[1])).map(([type, count]) => (
              <div key={type}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 140, fontSize: 13, fontWeight: 600, color: BLK, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>{type}</div>
                  <div style={{ flex: 1, height: 20, borderRadius: 4, background: '#f1f1f6', overflow: 'hidden' }}>
                    <div style={{ width: `${(Number(count) / maxTypeCount) * 100}%`, height: '100%', borderRadius: 4, background: "#0a0a0a", transition: 'width .4s ease', display: 'flex', alignItems: 'center', paddingLeft: 8 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: '#fff', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif" }}>{String(count)}</span>
                    </div>
                  </div>
                </div>
            ...
            <thead>
              <tr style={{ borderBottom: '2px solid #ececef' }}>
                {['URL', 'Recommended Schema', 'CTR Lift', ''].map(h => (
                  <th key={h} style={{ padding: '8px 10px', fontSize: 11, fontWeight: 800, color: '#1f2937', textTransform: 'uppercase', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", textAlign: h === 'URL' ? 'left' : 'center' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {eligible.slice(0, 20).map((opp, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f1f1f6' }}>
                  ...
                  <td style={{ textAlign: 'center', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 13, fontWeight: 800, color: GRN }}>{opp.potential_ctr_lift}</td>
                  <td style={{ textAlign: 'center' }}>
                    <button
      ...
      {schemaErrors.length > 0 && (
        <div style={{ ...card, borderLeft: `3px solid ${R}` }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => setExpandedErrors(!expandedErrors)}>
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 15, fontWeight: 800, color: R, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {semanticIssues.length > 0 && (
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => setExpandedSemantic(!expandedSemantic)}>
            <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 15, fontWeight: 800, color: BLK, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 16, fontWeight: 800, color: audit.semantic_html_score >= 70 ? GRN : AMB }}>{audit.semantic_html_score}/100</div>
