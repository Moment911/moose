    ...
    <div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 220 }}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 800, color: BLK, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#1f1f22', marginBottom: 6, fontFamily: DESIGN.fonts.body }}>Competitor Domains</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
            {domains.map(d => (
              <span key={d} style={{
                display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 10px',
                background: '#f1f1f6', color: T, borderRadius: 16, fontSize: 12, fontWeight: 700, fontFamily: DESIGN.fonts.body,
              }}>
                {d}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={domainInput} onChange={e => setDomainInput(e.target.value)}
              ...
              placeholder="competitor.com" style={{
                flex: 1, padding: '9px 12px', border: '1px solid #ececef', borderRadius: 8, fontSize: 13, fontFamily: DESIGN.fonts.body,
              }} />
            <button onClick={addDomain} style={{
              padding: '9px 16px', borderRadius: 8, border: '1px solid #ececef', background: '#fff',
              fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: DESIGN.fonts.body, display: 'flex', alignItems: 'center', gap: 4,
            }}>
              <Plus size={13} /> Add
            </button>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#1f1f22', marginBottom: 6, fontFamily: DESIGN.fonts.body }}>Email Alerts</div>
            ...
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="you@agency.com" disabled={!emailEnabled} style={{
              width: '100%', padding: '9px 12px', border: '1px solid #ececef', borderRadius: 8, fontSize: 13, fontFamily: DESIGN.fonts.body, boxSizing: 'border-box',
              opacity: emailEnabled ? 1 : 0.5,
            }} />
          </div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#1f1f22', marginBottom: 6, fontFamily: DESIGN.fonts.body }}>Slack Webhook (optional)</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#1f1f22', fontFamily: DESIGN.fonts.body }}>Frequency:</span>
            <select value={frequency} onChange={e => setFrequency(e.target.value)} style={{
              padding: '8px 12px', border: '1px solid #ececef', borderRadius: 8, fontSize: 13, fontWeight: 600, background: '#fff', fontFamily: DESIGN.fonts.body,
            }}>
      ...
      {watches.length > 0 && (
        <div style={card}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 800, color: BLK, marginBottom: 12 }}>Active Watches ({watches.length})</div>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 4, alignItems: 'center' }}>
            ...
            {[['all', 'All'], ['critical', 'Critical'], ['unread', 'Unread']].map(([val, lbl]) => (
              <button key={val} onClick={() => setFilter(val)} style={{
                padding: '4px 10px', borderRadius: 16, fontSize: 11, fontWeight: 700, fontFamily: DESIGN.fonts.body,
                ...
                <div key={ev.id || i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', background: '#f9f9fb', borderRadius: 10, borderLeft: `3px solid ${sev.color}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 3 }}>
                      <span style={{ fontSize: 11, fontWeight: 800, padding: '2px 8px', borderRadius: 10, background: sev.bg, color: sev.color, textTransform: 'uppercase', letterSpacing: '.04em', fontFamily: DESIGN.fonts.body }}>{sev.label}</span>
