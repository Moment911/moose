    ...
    <div>
      ...
      <div style={card}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
          ...
          {pos && (
            <div style={{
              ...
              fontSize: 13, fontWeight: 800, fontFamily: DESIGN.fonts.body, textTransform: 'uppercase', letterSpacing: '.05em',
            }}>
              {pos.label}
            </div>
          )}
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <input value={compInput} onChange={e => setCompInput(e.target.value)} placeholder="competitor1.com, competitor2.com, competitor3.com (optional — autodetects)" style={{
            flex: 1, padding: '10px 14px', border: '1px solid #ececef', borderRadius: 8, fontSize: 13, fontFamily: DESIGN.fonts.body, minWidth: 0,
          }} />
      ...
      {!data && !running && (
        <div style={{ ...card, textAlign: 'center', padding: 48 }}>
          ...
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 16, fontWeight: 700, color: BLK, marginBottom: 6 }}>No Scorecard Yet</div>
        ...
        <>
          {data.narrative && (
            <div style={{ ...card, background: 'linear-gradient(135deg,#f0f9ff 0%,#fefce8 100%)', borderLeft: `4px solid ${T}` }}>
              <div style={{ fontSize: 14, color: BLK, lineHeight: 1.65, fontFamily: DESIGN.fonts.body }}>{data.narrative}</div>
            </div>
          )}
          ...
          <div style={card}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #ececef' }}>
                    <th style={{ textAlign: 'left', padding: '10px 12px', fontSize: 11, fontWeight: 700, color: '#1f2937', textTransform: 'uppercase', fontFamily: DESIGN.fonts.body }}>Metric</th>
                    <th style={{ textAlign: 'center', padding: '10px 12px', fontSize: 11, fontWeight: 700, color: T, textTransform: 'uppercase', fontFamily: DESIGN.fonts.body }}>Client</th>
                    {competitors.map((c, i) => (
                      <th key={i} style={{ textAlign: 'center', padding: '10px 12px', fontSize: 11, fontWeight: 700, color: '#1f2937', textTransform: 'uppercase', fontFamily: DESIGN.fonts.body, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {c.domain}
                      </th>
                    ))}
                  </tr>
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 12, fontSize: 11, color: '#6b6b70', fontFamily: DESIGN.fonts.body }}>
