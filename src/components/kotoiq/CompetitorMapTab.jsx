        ...
        <>
          {data.central_entity && (
            <div style={{ ...card, borderLeft: `4px solid ${T}` }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#374151', textTransform: 'uppercase', marginBottom: 4 }}>Inferred Central Entity</div>
              <div style={{ fontFamily: FH, fontSize: 22, fontWeight: 800, color: T }}>{data.central_entity}</div>
              {data.niche && <div style={{ fontSize: 13, color: '#374151', marginTop: 4 }}>Niche: {data.niche}</div>}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div style={card}>
              ...
              {(data.core_topics || []).map((t, i) => (
                <div key={i} style={{ padding: '8px 12px', background: R + '08', borderRadius: 6, marginBottom: 4, fontSize: 12, color: '#374151' }}>
                  {typeof t === 'string' ? t : t.topic || t.name}
                </div>
              ))}
              {!data.core_topics?.length && <div style={{ fontSize: 12, color: '#374151' }}>None detected</div>}
            </div>
            ...
            <div style={card}>
              ...
              {(data.outer_topics || []).map((t, i) => (
                <div key={i} style={{ padding: '8px 12px', background: AMB + '08', borderRadius: 6, marginBottom: 4, fontSize: 12, color: '#374151' }}>
                  {typeof t === 'string' ? t : t.topic || t.name}
                </div>
              ))}
              {!data.outer_topics?.length && <div style={{ fontSize: 12, color: '#374151' }}>None detected</div>}
            </div>
          </div>
            ...
            <div style={card}>
              ...
              {data.coverage_comparison.missing_topics?.length > 0 && (
                <div style={{ marginTop: 12 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#374151', textTransform: 'uppercase', marginBottom: 6 }}>Topics competitor covers that you don't</div>
          ...
          {data.strategic_insights?.length > 0 && (
            <div style={card}>
              ...
              {data.strategic_insights.map((s, i) => (
                <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 12px', background: '#f9fafb', borderRadius: 8, marginBottom: 4 }}>
                  <Lightbulb size={14} color={AMB} style={{ marginTop: 2 }} />
                  <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.5 }}>{typeof s === 'string' ? s : s.insight || s.text}</div>
                </div>
              ))}
            </div>
          )}
