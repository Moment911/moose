...
const EXPLANATIONS = {
  ...
  bulk_ops: {
    layman: `Managing 20+ clients? Pick an operation — on-page audit, content briefs, topical map update, content refresh, or full sync — and run it across every client (or just the ones you select) in one click. You watch real-time progress per client and get a summary once everything finishes.`,
    technical: `Creates a kotoiq_bulk_runs row with operation + options, then a kotoiq_bulk_run_clients row per targeted client (status: queued). For each client, fires an internal POST to the matching /api/kotoiq action (analyze_on_page, generate_brief, build_content_inventory, generate_topical_map, or sync) fire-and-forget. Worker updates per-client status (running → complete/failed) with result_summary + error. Rollup counters (completed/failed/total) are recomputed from children after each completion. UI polls get_bulk_operation_status every 3s until in_progress = 0.`,
    data_sources: ['clients table', 'kotoiq_bulk_runs', 'kotoiq_bulk_run_clients', 'kotoiq_processing_jobs', 'All downstream engines (on_page, briefs, topical, refresh, sync)'],
    output_layman: `**Progress Bar:** X of Y clients complete.\n\n**Per-Client Status:** Each client shows spinner/check/x with a short result summary once done (e.g., "briefs_created: 10" or "keywords: 342").\n\n**Errors:** Any client that failed shows the error inline — no retrying required, just rerun for those clients.\n\n**Next Action:** Once complete, open any client to see the results inside the normal KotoIQ tabs.`,
    output_technical: `**bulk_run.status:** running | complete\n**per-client status:** queued | running | complete | failed\n**result_summary:** operation-specific (briefs_created, pages_scanned, nodes, coverage, keywords, score, issues)\n**polling cadence:** 3s via get_bulk_operation_status\n**termination:** bulk_run.status = 'complete' when all children terminal`,
  },
  client_portal: {
    layman: `A beautiful, white-label page you can send directly to your client. It shows their AI Visibility Score, total keywords, top 10 rankings, topical authority, E-E-A-T grade, recent wins, top opportunities, content published this month, their position vs top 3 competitors, and your recommended next actions — read-only and branded in your agency's colors.`,
    technical: `Public route /portal/[clientId] (no auth). POSTs to action 'get_portal_data' which runs getPortalData() — 11 parallel queries across kotoiq_ai_visibility_snapshots, kotoiq_keywords (counts + striking distance), kotoiq_topical_maps, kotoiq_eeat_audit, kotoiq_snapshots (45d window for win detection), kotoiq_content_calendar (status='published' this month), kotoiq_scorecards (competitor breakdown), kotoiq_quick_win_queue (status='pending'). Branding resolved from agencies.primary_color/secondary_color/brand_logo_url/metadata with Koto fallback. Rate-limited to 30 req/min per IP. Every view logged to kotoiq_portal_views.`,
    data_sources: ['clients', 'agencies (branding)', 'kotoiq_ai_visibility_snapshots', 'kotoiq_keywords', 'kotoiq_topical_maps', 'kotoiq_eeat_audit', 'kotoiq_snapshots', 'kotoiq_content_calendar', 'kotoiq_scorecards', 'kotoiq_quick_win_queue'],
    output_layman: `**Public URL:** /portal/{clientId} — shareable, read-only.\n\n**White-Label:** Your agency's primary color + logo (falls back to Koto brand if not set).\n\n**Contents:** AI Visibility hero, 4 KPI cards, recent wins, top opportunities, content published, competitor landscape, next actions, request-more-details CTA.\n\n**Next Action:** Click "Share Portal Link" on the KotoIQ dashboard to copy the URL to clipboard and send to your client.`,
    output_technical: `**route:** /portal/[clientId] (dynamic, client component)\n**API:** POST /api/kotoiq { action: 'get_portal_data', client_id }\n**rate limit:** 30 req/min/IP (in-memory token bucket)\n**view tracking:** kotoiq_portal_views (client_id, viewer_ip, viewer_user_agent, viewed_at)\n**branding precedence:** agencies.primary_color → metadata.primary_color → '#00C2CB' fallback`,
  },
  strategic_plan: {
    layman: `Instead of running individual tools and trying to figure out what to do next, KotoIQ looks at every piece of data across your site — rankings, content, links, technical health, competitors — and gives you one prioritized plan. It tells you what to attack (topics where you can realistically win), what to defend (where you're strong and need to protect your lead), and what to abandon (lost causes draining your budget). You also get a week-by-week action list and monthly milestones so the plan actually gets executed.`,
    technical: `generateStrategicPlan aggregates inputs from kotoiq_topical_maps, kotoiq_keywords (position distribution, opportunity scores), kotoiq_backlink_profile, kotoiq_eeat_audit, kotoiq_brand_serp, kotoiq_content_inventory, kotoiq_competitor_events, and kotoiq_scorecards. Builds a structured prompt for Claude Sonnet 4 with the client's competitive posture and timeframe (1/3/6 month). Claude returns attack_priorities, defend_priorities, abandon list, weekly_actions, monthly_milestones, and resource_allocation (content/links/technical/local %). Persists to kotoiq_strategic_plans with status='active'.`,
    data_sources: ['kotoiq_topical_maps', 'kotoiq_keywords', 'kotoiq_backlink_profile', 'kotoiq_eeat_audit', 'kotoiq_scorecards', 'Claude Sonnet 4'],
    output_layman: `**Attack Priorities:** Clusters where you have a realistic shot — each with a "why attack" reason, estimated impact, and specific weekly action.\n\n**Defend Priorities:** Clusters where you're already winning — defense strategies to hold position.\n\n**Abandon List:** Topics you're wasting budget on — Claude recommends cutting.\n\n**Resource Allocation:** How to split your hours — content vs links vs technical vs local — in percent.\n\n**Weekly Actions:** Concrete tasks grouped by week with owners and estimated hours.\n\n**Monthly Milestones:** The checkpoints — each with a specific metric to hit.\n\n**Next Action:** Work the weekly actions top-down, check off each week, hit every milestone.`,
    output_technical: `**attack_priorities[]:** {cluster, why_attack, estimated_impact, weekly_action}\n**defend_priorities[]:** {cluster, defense_strategy}\n**abandon[]:** {cluster, reason}\n**weekly_actions[]:** {week: int, action, owner?, estimated_hours?}\n**monthly_milestones[]:** {month: int, milestone, metric}\n**resource_allocation:** {content_pct, links_pct, technical_pct, local_pct} — must sum to 100\n**timeframes:** '1_month' | '3_month' | '6_month'`,
  },
  scorecard: {
    layman: `A side-by-side scorecard comparing your SEO to up to 3 competitors across 12 weighted metrics — Domain Authority, backlinks, content depth, E-E-A-T, schema, Brand SERP, publishing velocity, Core Web Vitals, AEO readiness, and more. Every cell is color-coded: green where you win, amber where you're level, red where you lose. You get one headline label — Leader, Contender, Challenger, or Behind — plus the priority gaps to close.`,
    technical: `generateScorecard pulls 12 metrics from kotoiq_topical_maps, kotoiq_eeat_audit, kotoiq_schema_audit, kotoiq_backlink_profile, kotoiq_brand_serp, kotoiq_content_inventory, kotoiq_grid_scans_pro, kotoiq_technical_deep, kotoiq_aeo_audits. For each competitor domain (auto-detected from kotoiq_competitors if not supplied), fetches live Moz metrics, homepage signals, and sitemap size in parallel. Gap analysis weights metrics by impact tier (critical/high/medium/low) and normalizes percent gap. Claude Sonnet 4 produces narrative + overall_position + recommended_focus_areas. Persisted to kotoiq_scorecards.`,
    data_sources: ['12 KotoIQ metric tables', 'Moz API v2 (url_metrics)', 'Competitor sitemaps', 'DataForSEO (ranked keywords)', 'Claude Sonnet 4'],
    output_layman: `**Overall Position:** Leader (>strengths than gaps) | Contender (few gaps) | Challenger (some critical gaps) | Behind (many critical gaps).\n\n**Comparison Table:** 12 metric rows × up to 4 columns (you + competitors). Green = winning, amber = level, red = losing.\n\n**Strengths:** Metrics where you lead the pack.\n\n**Priority Focus Areas:** The gaps to close first — ordered by impact × feasibility.\n\n**Narrative:** Claude's plain-English summary of the competitive picture.\n\n**Next Action:** Click any red cell → it's a gap. Feed the top 3 into the Strategic Plan tab.`,
    output_technical: `**client_scores:** {metric: value} for 12 metrics\n**competitor_scores[]:** per-domain metric arrays with live Moz + sitemap + DataForSEO data\n**gaps[]:** {metric, client_value, competitor_best, gap_size, impact: critical|high|medium|low}\n**strengths[]:** metric names where client leads\n**overall_position:** leader | contender | challenger | behind\n**weighting tiers:** critical (DA, referring_domains) | high (topical_authority, E-E-A-T, content_depth) | medium (schema, brand_serp, solv) | low (toxic_pct, total_pages)`,
  },
  competitor_watch: {
    layman: `Instead of manually checking your competitors every week, this watches them for you. It tracks their sitemap, their keyword rankings, and their backlink profile — and pings you the moment they publish new content, gain a ranking, or pick up new links. You pick the competitor domains, the alert channels (email / Slack), and the check frequency, then forget about it. Everything important lands in your feed with a severity label.`,
    technical: `setupCompetitorWatch inserts to kotoiq_competitor_watches with competitor_domains[], alert_channels {email, slack_webhook, teams_webhook}, check_frequency (daily|weekly). runCompetitorWatchCheck: for each active watch, fetches competitor sitemap URLs, diffs against prior snapshot in kotoiq_competitor_url_snapshots, calls DataForSEO getDomainRankedKeywords (top 50), detects new_top_10 entries and 5+ position improvements. SERP movement cross-referenced against client's top 5 keywords. Events written to kotoiq_competitor_events with severity (critical/warning/info). Alerted events get marked alerted=true.`,
    data_sources: ['Competitor sitemaps', 'DataForSEO (domain ranked keywords)', 'kotoiq_competitor_url_snapshots', 'Slack/Email/Teams webhooks', 'Claude Sonnet 4 (summaries)'],
    output_layman: `**Event Feed:** Every detected change with severity badge (Critical / Warning / Info), competitor domain, event type, and a one-line summary.\n\n**Event Types:** New Content (sitemap URLs added), Ranking Gains (competitor moved up), New Backlinks (fresh link pickups), SERP Movement (shifts on your tracked keywords).\n\n**Severity:** Critical = they just took a top-10 position from you. Warning = meaningful gain. Info = routine change.\n\n**Filters:** All / Critical Only / Unread.\n\n**Next Action:** Critical event → pivot that week's plan. Warning → queue a response. Info → track.`,
    output_technical: `**event_types:** new_content | ranking_gains | new_backlinks | serp_movement\n**severity:** critical (top-10 keyword stolen) | warning (5+ position gain) | info (routine)\n**detection thresholds:** new_top_10 (prev absent, current pos ≤ 10) | improved (prev_pos − cur_pos ≥ 5)\n**snapshot diff:** currentUrls − prevUrls via sitemap regex parse\n**alerted flag:** true once delivered to any channel (email / slack / teams)\n**check_frequency:** daily | weekly — scheduled via cron`,
  },
  integrations: {
    layman: `Pipe KotoIQ alerts into the tools your team already uses — Slack, Microsoft Teams, or email. Pick your alert types (ranking changes, competitor moves, new reviews, weekly digest, monthly report) and the platform handles delivery automatically. Includes a one-click "Send Daily Digest Now" button so you can ping the team on demand, plus a full alert history so you can see what went where.`,
    technical: `setupSlackIntegration / setupTeamsIntegration write to kotoiq_integrations with webhook_url, alert_types[], and scope (client_id OR agency_id). sendSlackAlert and sendTeamsAlert POST platform-appropriate message cards to the webhook URL. sendDailyDigest aggregates last-24h kotoiq_snapshots, kotoiq_competitor_events, kotoiq_content_inventory, and reviews, then asks Claude Sonnet 4 for a 5-bullet Slack-markdown summary. Cron triggers at 9 AM client timezone for daily digests. Token usage logged to koto_token_usage with feature=daily_digest.`,
    data_sources: ['Slack Incoming Webhooks', 'Microsoft Teams Incoming Webhooks', 'kotoiq_integrations (config)', 'Claude Sonnet 4 (digest summaries)', 'Cron (Vercel scheduled)'],
    output_layman: `**Connected Channels:** Each integration shows a status badge (Connected / Not Connected), webhook URL, and the alert types it's subscribed to.\n\n**Test Message:** Sends a sample daily digest through the webhook so you can verify the plumbing works.\n\n**Send Daily Digest Now:** Manually trigger the digest — useful for end-of-day wrap or when you need to ping the team immediately.\n\n**Schedule:** Daily digests fire at 9 AM in the client's timezone. Weekly digests go Monday. Monthly reports ship the 1st.\n\n**Alert History:** Timestamped log of every message sent with delivery status.\n\n**Next Action:** Paste the webhook URL, pick your alert types, hit Save, then send a test to confirm.`,
    output_technical: `**integration_types:** slack | teams\n**scope:** client_id (per-client) | agency_id (agency-wide aggregate)\n**alert_types:** ranking_changes | competitor_events | reviews | weekly_digest | monthly_report\n**webhook formats:** Slack Block Kit with sections/dividers/fields; Teams MessageCard with themeColor + sections\n**cron_schedule:** daily 9:00 client_tz | weekly Monday 9:00 | monthly 1st 9:00\n**digest payload:** {ranking_snapshots, competitor_events, content_published, reviews_received} → Claude 5-bullet summary`,
  },
  autopilot: {
    layman: `Give KotoIQ one target keyword and it runs the entire content pipeline for you — 8 steps, start to finish. It analyzes query gaps, maps the semantic frames, suggests must-include entities, generates the brief, writes the draft, humanizes the language, runs quality checks (topicality, plagiarism, on-page score), and attaches JSON-LD schema. If the Human Score clears your bar, it can auto-publish.`,
    technical: `runAutonomousPipeline orchestrates 8 phases against a single kotoiq_pipeline_runs row for observability: (1) runQueryGapAnalyzer, (2) runFrameAnalyzer, (3) runNamedEntitySuggester, (4) brief generation via Claude Sonnet 4, (5) full-content generation (structured macro/micro sections + must-include entity injection), (6) humanization pass (runContextlessWordRemover → runMetadiscourseAuditor → runSentenceFilterer → runEntityInserter), (7) quality checks (topicality, plagiarism via DataForSEO phrase-match, on-page score), (8) runTripleGenerator schema emission. Each step persisted with duration_ms and status. If auto_publish and human_score >= 85, writes to CMS via publishing adapter.`,
    data_sources: ['kotoiq_keywords', 'kotoiq_pipeline_runs', '12 semantic agents', 'DataForSEO SERP', 'Claude Sonnet 4', 'Publishing adapter (optional)'],
    output_layman: `**Human Score (0-100):** How human the content reads. 85+ = ship it.\n\n**Topicality Score:** How fully it covers what Google expects for the keyword.\n\n**Originality Score:** 100 − duplication found via SERP phrase-matching.\n\n**On-Page Score:** 17-check on-page SEO audit.\n\n**Auto-published URL:** If your Human Score passed 85 and auto-publish was on, the live URL.\n\n**Next Action:** Flagged → review reasons list. Completed but not auto-published → click "Publish Now".`,
    output_technical: `**status:** completed | flagged | failed\n**steps[]:** [{step, status, duration_ms, output_preview, error?}]\n**human_score + topicality_score + plagiarism_score + on_page_score:** 0-100\n**schema_json_ld[]:** array of JSON-LD @graph nodes ready to paste into <script type="application/ld+json">\n**auto_publish threshold:** human_score >= 85 AND flagged_reasons.length === 0\n**flagged_reasons[]:** rules that blocked auto-publish (e.g. topicality < 70)`,
  },
  knowledge_graph: {
    layman: `The Knowledge Panel — that big box about your business on the right side of Google — doesn't happen by accident. It's triggered when Google connects enough structured entity data across Wikidata, schema.org, reviews, and authoritative citations. This tool generates the three most powerful entity-graph formats (Wikidata QuickStatements, schema.org JSON-LD, RDF Turtle), computed from every fact we know about your business, plus a step-by-step submission guide.`,
    technical: `exportKnowledgeGraph gathers entity facts from clients + kotoiq_review_intelligence + kotoiq_schema_audit + kotoiq_backlink_profile + kotoiq_topical_maps + kotoiq_brand_serp + kotoiq_sync_log. Computes deterministic scoreKnowledgePanelLikelihood (0-100) weighted across: basic entity completeness (name, website, phone, address, industry), schema presence, domain authority + edu/gov links, topical authority score, review volume, existing KP flag. Claude Sonnet 4 emits the requested format (Wikidata QuickStatements v1 with P-codes, schema.org JSON-LD Organization/LocalBusiness, or RDF/Turtle using schema + foaf vocabularies) with submission guide. Persists to kotoiq_knowledge_graph_exports.`,
    data_sources: ['clients', 'kotoiq_review_intelligence', 'kotoiq_schema_audit', 'kotoiq_backlink_profile', 'kotoiq_topical_maps', 'kotoiq_brand_serp', 'Claude Sonnet 4'],
    output_layman: `**KP Trigger Likelihood (0-100):** How likely Google is to surface a Knowledge Panel for you based on current signals.\n\n**Entity Properties:** Every known fact (name, phone, address, industry, website) tagged by source.\n\n**Generated Content:** Copy-paste Wikidata statements, JSON-LD, or RDF/Turtle.\n\n**Related Entities:** Connections to city QIDs, industry QIDs, related organizations.\n\n**Submission Guide:** Step-by-step instructions for how to publish it (Wikidata submission, JSON-LD placement, Turtle endpoint).\n\n**Next Action:** Copy the content, follow the guide, re-run Brand SERP in 4-8 weeks to measure KP appearance.`,
    output_technical: `**kp_likelihood:** deterministic score (name 8 + website 8 + phone 6 + address 8 + industry 6 + schema 10 + DA × 0.3 + edu_gov 5 + topical × 0.1 + reviews up to 10 + has_KP 15)\n**format:** wikidata | json_ld | rdf_turtle\n**entity_properties[]:** [{name, value, source}] — only real data, never fabricated\n**submission_guide_markdown:** format-specific step-by-step\n**Persistence:** kotoiq_knowledge_graph_exports (id, format, content, kp_likelihood, submitted_to_wikidata, wikidata_entry_id)`,
  },
  hyperlocal: {
    layman: `Local rankings vary block by block. If Rank Grid Pro shows you're invisible in certain neighborhoods (dead zones where you rank below 20), this tool creates hyperlocal content to fix it. It reverse-geocodes every dead zone to find the neighborhood, clusters nearby points into neighborhood groups, then generates a landing-page-ready content brief per neighborhood — with local entity markers (landmarks, neighborhood name, nearest intersections) baked in.`,
    technical: `generateHyperlocalFromGrid reads the latest kotoiq_grid_scans_pro row (or a specified grid_scan_id), filters to dead zones (position > 20), reverse-geocodes each via Google Geocoding API to extract neighborhood/sublocality/locality. Clusters dead points using haversine distance (default 1km radius). For each neighborhood cluster: Claude Sonnet 4 generates a content brief with hyperlocal schema (LocalBusiness + address + geo coordinates + landmarks), target URL slug, and suggested keyword. Briefs persisted to kotoiq_content_briefs; optionally added to kotoiq_content_calendar. Returns estimated coverage improvement based on closed dead zones.`,
    data_sources: ['kotoiq_grid_scans_pro (dead zones)', 'Google Geocoding API (reverse geocode)', 'kotoiq_content_briefs', 'kotoiq_content_calendar', 'Claude Sonnet 4'],
    output_layman: `**Dead Zones Analyzed:** Grid points where your rank was worse than 20.\n\n**Neighborhoods Identified:** Named neighborhoods where dead zones cluster — each becomes one landing page target.\n\n**Briefs Created:** Ready-to-write content briefs per neighborhood with target keyword, URL, and schema.\n\n**Estimated Coverage Improvement:** How much Share of Local Voice you'll gain if you publish every brief.\n\n**Next Action:** Click "Open Brief" to review and refine each one, then pass to your writer or the Auto-Pilot Pipeline.`,
    output_technical: `**dead_zones_analyzed:** count of grid points where rank > 20\n**neighborhoods_identified[]:** [{name, weak_points, avg_rank, population_estimate?}]\n**briefs_created[]:** [{brief_id, neighborhood, target_keyword, target_url, schema_preview}]\n**clustering:** haversine distance <= 1km groups dead points into a single neighborhood\n**estimated_coverage_improvement:** projected SoLV% gain if all briefs are published`,
  },
  sitemap_crawler: {
    layman: `Every other engine in KotoIQ needs to know what pages exist on your site. This crawler handles massive, real-world sites — including ones with multi-level sitemap indexes, gzipped sitemaps, and 10,000+ URLs. Point it at your site once and every downstream tool (schema audit, on-page, internal links, content inventory, semantic) can process every URL.`,
    technical: `crawlSitemaps discovers root sitemaps (/sitemap.xml, /sitemap_index.xml, robots.txt sitemap directives), follows <sitemapindex> children recursively up to maxDepth, handles gzip-encoded sitemaps (via gunzipSync), extracts <url> blocks with loc + lastmod + priority + changefreq. Supports filters: maxUrls cap, includePatterns/excludePatterns substring match, onlyNewSince for incremental crawls. Persists crawl progress to kotoiq_sitemap_crawls and every URL to kotoiq_sitemap_urls. processUrlsInChunks utility lets downstream engines process URL batches with configurable concurrency without timing out.`,
    data_sources: ['Root sitemap.xml', 'Sitemap index files', 'robots.txt sitemap directives', 'Gzipped sitemaps (.xml.gz)'],
    output_layman: `**Total URLs Discovered:** Every single page your sitemap claims exists.\n\n**Live Progress:** Sitemaps found vs processed, URLs discovered vs saved, depth reached — updated every 2 seconds while crawling.\n\n**URL Explorer:** Paginated, sortable table showing URL, lastmod, priority, changefreq, and source sitemap.\n\n**Filters:** Narrow by path (e.g. /blog/), by lastmod date.\n\n**Next Action:** Export CSV for analysis. Any downstream engine (schema, on-page, internal links, semantic) can now pull from this URL list.`,
    output_technical: `**sitemaps_found vs sitemaps_processed:** root + child sitemap counts\n**urls_discovered vs urls_saved:** pre- and post-deduplication\n**depth_reached:** sitemap-index recursion depth\n**errors[]:** per-sitemap fetch/parse errors\n**options.maxUrls:** stop cap (default unlimited)\n**options.maxDepth:** recursion limit (default 10)\n**options.includePatterns / excludePatterns:** substring match filters\n**options.onlyNewSince:** ISO date → only URLs with lastmod >= this`,
  },
  jobs: {
    layman: `Some engines in KotoIQ need to process thousands of URLs — enough that one HTTP request can't finish them. This panel shows every background job that's running, queued, or done. You can filter by status, see progress bars per job, and check elapsed time. If a job is running, this panel auto-refreshes every 5 seconds.`,
    technical: `Reads kotoiq_processing_jobs — jobs are created via action=create_processing_job (engine, total_urls, batch_size, concurrency, metadata) and advanced by background workers that update processed_urls + status in batches. Status values: queued | running | complete | failed. get_processing_jobs supports filtering by engine + status. UI polls every 5s when any job is running; expand any job to see metadata and errors array.`,
    data_sources: ['kotoiq_processing_jobs'],
    output_layman: `**Job Row:** Shows engine, status badge, progress (processed / total), elapsed time, errors indicator.\n\n**Status Filters:** All | Running | Queued | Complete | Failed.\n\n**Auto-refresh:** Every 5 seconds when any job is running.\n\n**Expand Row:** See full metadata JSON and error details.\n\n**Next Action:** Queued jobs will start automatically. Failed jobs → investigate errors → rerun the source engine.`,
    output_technical: `**status:** queued | running | complete | failed\n**progress:** processed_urls / total_urls (pct derived)\n**elapsed:** started_at → (completed_at || now)\n**poll_interval:** 5s when any job.status === 'running'\n**engine filter:** set(kotoiq_processing_jobs.engine)\n**metadata jsonb:** engine-specific payload (target_urls, batch params, etc.)`,
  },
  aeo: {
