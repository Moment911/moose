...
export default function BingAuditTab({ clientId, agencyId }) {
  ...
  return (
    <div>
      ...
      <div style={card}>
        <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 18, fontWeight: 800, color: BLK, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
          ...
          <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="Bing Webmaster Tools API key" style={{
            flex: 1, padding: '10px 14px', border: '1px solid #ececef', borderRadius: 8, fontSize: 13, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif",
          }} />
        </div>
        <button onClick={run} disabled={running} style={{
          display: 'flex', alignItems: 'center', gap: 6, padding: '10px 22px', borderRadius: 8,
          border: 'none', background: '#0078D4', color: '#fff', fontSize: 13, fontWeight: 700, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif",
      ...
      {data && (
        <>
          ...
          {data.top_queries?.length > 0 && (
            <div style={card}>
              <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 15, fontWeight: 800, color: BLK, marginBottom: 12 }}>Top Bing Queries</div>
...
function Stat({ label, value, color }) {
  return (
    <div style={{ background: '#f9f9fb', borderRadius: 10, padding: '14px 18px', border: '1px solid #ececef' }}>
      ...
      <div style={{ fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, system-ui, sans-serif", fontSize: 22, fontWeight: 900, color }}>{value}</div>
    </div>
  )
}
