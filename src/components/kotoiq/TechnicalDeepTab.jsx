  ...
  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontFamily: DESIGN.fonts.body, fontSize: size * 0.32, fontWeight: 900, color: c, lineHeight: 1 }}>{score}</span>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: c + '0a', borderRadius: 10, border: `1px solid ${c}20` }}>
      ...
      <div>
        ...
        <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 18, fontWeight: 900, color: c }}>{score}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ flex: 1 }}>
      {label && <div style={{ fontSize: 11, color: '#1f1f22', marginBottom: 4, display: 'flex', justifyContent: 'space-between' }}>
        <span>{label}</span><span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700 }}>{Math.round(pct)}%</span>
      </div>}
  ...
  if (!data) return (
    <div style={card}>
      <div style={{ textAlign: 'center', padding: '40px 20px' }}>
        ...
        <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 18, fontWeight: 800, color: BLK, marginBottom: 8 }}>Technical SEO Deep Audit</div>
    ...
    <>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 28, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 18, fontWeight: 800, color: BLK, marginBottom: 4 }}>Technical SEO Score</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        {/* Crawl Budget */}
        <div style={card}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            <div>
              ...
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 24, fontWeight: 900, color: BLK }}>{(d.total_urls || 0).toLocaleString()}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#1f2937', fontWeight: 700, textTransform: 'uppercase' }}>Indexable</div>
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 24, fontWeight: 900, color: GRN }}>{(d.indexable_urls || 0).toLocaleString()}</div>
            </div>
          </div>
        ...
        {/* Canonical Health */}
        <div style={card}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK, marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <FileText size={15} color={AMB} /> Canonical Health
              <span style={{ fontFamily: DESIGN.fonts.body, fontSize: 18, fontWeight: 900, color: d.canonical_score >= 70 ? GRN : d.canonical_score >= 40 ? AMB : R, marginLeft: 8 }}>
                {d.canonical_score}
              </span>
            </div>
        ...
        <div style={card}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK, marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontFamily: DESIGN.fonts.body, fontSize: 18, fontWeight: 900, color: d.mobile_score >= 70 ? GRN : d.mobile_score >= 40 ? AMB : R, marginLeft: 8 }}>
                {d.mobile_score}
              </span>
            </div>
            ...
            {Object.entries(statusDist).sort((a, b) => b[1] - a[1]).map(([code, count]) => {
              ...
              return (
                <div key={code} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <span style={{ fontFamily: DESIGN.fonts.body, fontSize: 12, fontWeight: 800, color, width: 36 }}>{code}</span>
                  <div style={{ flex: 1, height: 6, borderRadius: 3, background: '#f1f1f6', overflow: 'hidden' }}>
                    <div style={{ width: `${pct}%`, height: '100%', borderRadius: 3, background: color }} />
                  </div>
                  <span style={{ fontSize: 11, color: '#1f1f22', fontFamily: DESIGN.fonts.body, fontWeight: 600, width: 55, textAlign: 'right' }}>{count} ({pct}%)</span>
                </div>
              )
            })}
              ...
              <div style={{ marginTop: 12 }}>
                ...
                {Object.entries(d.not_indexed_reasons).map(([reason, count]) => (
                  <div key={reason} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#1f1f22', marginBottom: 4 }}>
                    ...
                    <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, marginLeft: 'auto' }}>{count}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Core Web Vitals */}
      <div style={card}>
        <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
          {[
            ...
            <div key={key} style={{ textAlign: 'center', padding: '16px 12px', background: '#fafafb', borderRadius: 10 }}>
              ...
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 32, fontWeight: 900, color: cwvColor(value, key) }}>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 22, fontWeight: 900, color: BLK }}>{(d.sitemap_urls_count || 0).toLocaleString()}</div>
          </div>
          <div>
          ...
          <div style={{ marginTop: 12, borderTop: '1px solid #f1f1f6', paddingTop: 10 }}>
            {(d.sitemap_issues || []).map((issue, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', fontSize: 12, color: '#1f1f22', borderBottom: '1px solid #f9f9fb' }}>
                ...
                {issue.status && <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 11, color: R }}>{issue.status}</span>}
              </div>
            ))}
          </div>
        )}
