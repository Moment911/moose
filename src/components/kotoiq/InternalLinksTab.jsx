  ...
  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontFamily: DESIGN.fonts.body, fontSize: size * 0.32, fontWeight: 900, color, lineHeight: 1 }}>{score}</span>
...
function StatBox({ label, value, icon: Icon, color }) {
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #ececef', padding: '14px 16px', flex: 1, minWidth: 120 }}>
      ...
      <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 22, fontWeight: 900, color: BLK }}>{value}</div>
    </div>
  )
}
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #ececef', overflow: 'hidden' }}>
      <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: items.length > 0 ? '1px solid #f1f1f6' : 'none' }}>
        ...
        <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 13, color: BLK }}>{title}</span>
        <span style={{
          marginLeft: 'auto', fontFamily: DESIGN.fonts.body, fontSize: 11, fontWeight: 800, padding: '2px 8px',
...
function EquityBar({ url, count, maxCount, isOverLinked, isStarved }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0' }}>
      ...
      <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 12, fontWeight: 700, color, width: 36, textAlign: 'right', flexShrink: 0 }}>{count}</div>
    </div>
  )
}
  ...
  if (!a) {
    return (
      <div style={{ maxWidth: 520, margin: '60px auto', textAlign: 'center' }}>
        ...
        <h2 style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 8 }}>Internal Link Intelligence</h2>
        ...
        <button onClick={runScan} disabled={scanning} style={{
          ...
          fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 13, cursor: scanning ? 'wait' : 'pointer',
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <h2 style={{ fontFamily: DESIGN.fonts.body, fontSize: 18, fontWeight: 800, color: BLK, margin: 0 }}>Internal Link Score</h2>
        ...
        <button onClick={runScan} disabled={scanning} style={{
          ...
          fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 12, cursor: scanning ? 'wait' : 'pointer',
          ...
          renderItem={(item) => (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#1f1f22', textDecoration: 'none' }}
                onMouseEnter={e => e.currentTarget.style.color = T} onMouseLeave={e => e.currentTarget.style.color = '#1f1f22'}
              >{shortUrl(item.url)}</a>
              <span style={{ fontFamily: DESIGN.fonts.body, fontSize: 11, fontWeight: 700, color: R }}>{item.link_count} links</span>
            </div>
          )}
        />
      </div>
      ...
      {topEquity.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #ececef', padding: '18px 22px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 14, color: BLK }}>Link Equity Distribution</span>
      ...
      {qualityNodes.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #ececef', padding: '18px 22px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 14, color: BLK }}>Quality Node Suggestions</span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ background: '#f9f9fb' }}>
                  <th style={{ textAlign: 'left', padding: '8px 12px', fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 12, color: '#1f2937', textTransform: 'uppercase' }}>Page</th>
                  <th style={{ textAlign: 'center', padding: '8px 12px', fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 12, color: '#1f2937', textTransform: 'uppercase' }}>Clicks</th>
                  <th style={{ textAlign: 'center', padding: '8px 12px', fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 12, color: '#1f2937', textTransform: 'uppercase' }}>Inbound Links</th>
                  <th style={{ textAlign: 'center', padding: '8px 12px', fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 12, color: '#1f2937', textTransform: 'uppercase' }}>Keywords</th>
                  <th style={{ textAlign: 'center', padding: '8px 12px', fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 12, color: '#1f2937', textTransform: 'uppercase' }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {qualityNodes.map((node, i) => (
                  <tr key={i} style={{ borderTop: '1px solid #f1f1f6' }}>
                    ...
                    <td style={{ padding: '8px 12px', textAlign: 'center', fontFamily: DESIGN.fonts.body, fontWeight: 700, color: GRN }}>{fmtN(node.organic_clicks)}</td>
                    <td style={{ padding: '8px 12px', textAlign: 'center', fontFamily: DESIGN.fonts.body, fontWeight: 700, color: node.inbound_links < 3 ? '#e9695c' : BLK }}>{node.inbound_links}</td>
      ...
      <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #ececef', padding: '18px 22px', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 14, color: BLK }}>Breadcrumb Coverage</span>
          <span style={{
            marginLeft: 8, fontFamily: DESIGN.fonts.body, fontSize: 13, fontWeight: 800,
      ...
      {recommendations.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #ececef', padding: '18px 22px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 14, color: BLK }}>AI Recommendations</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {recommendations.map((rec, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 800, color: T, flexShrink: 0 }}>{i + 1}.</span>
                {rec}
              </div>
            ))}
          </div>
      ...
      {(suggestLoading || suggestions) && (
        <div style={{ background: '#fff', borderRadius: 16, border: `2px solid ${T}`, padding: '18px 22px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <span style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, fontSize: 14, color: BLK }}>Link Suggestions for {shortUrl(suggestUrl)}</span>
