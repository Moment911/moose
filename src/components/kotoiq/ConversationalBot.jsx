          ...
          {showBadge ? (
            <span
              ...
              style={{
                ...
                fontFamily: DESIGN.fonts.body, fontSize: 11, fontWeight: 700,
    ...
    return (
      ...
      >
          ...
          {showBadge ? (
            <span
              ...
              style={{
                ...
                width: 18, height: 18, borderRadius: 9,
                background: "#0a0a0a", color: '#fff',
                fontFamily: DESIGN.fonts.body, fontSize: 10, fontWeight: 700,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                border: '2px solid #fff',
              }}
            >{badgeText}</span>
        ...
        <div style={{
          ...
          fontFamily: DESIGN.fonts.body, fontSize: 12, fontWeight: 700, letterSpacing: 1.2,
        }}>
          {mode === 'clarifications'
  ...
  return (
    <div style={{ ...panelStyle, background: '#fff', border: '1px solid #ececef', boxShadow: '0 20px 50px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #ececef', display: 'flex', alignItems: 'center', gap: 10, background: BLK, color: '#fff' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 700, fontFamily: DESIGN.fonts.body }}>
            {mode === 'clarifications' ? 'Clarifications' : 'KotoIQ Assistant'}
          </div>
          <div style={{ fontSize: 11, opacity: 0.75, fontFamily: DESIGN.fonts.body, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      {mode === 'clarifications' && !showHistory ? (
        <div style={{ flex: 1, overflowY: 'auto', padding: 16, background: GRY }}>
          {clarifLoading ? (
            <div style={{ textAlign: 'center', padding: '24px 8px', color: '#6b6b70', fontSize: 13, fontFamily: DESIGN.fonts.body }}>
            ...
            <div style={{ textAlign: 'center', padding: '40px 12px' }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: BLK, fontFamily: DESIGN.fonts.body, marginBottom: 6 }}>
                All clear.
              </div>
              <div style={{ fontSize: 13, color: '#6b6b70', fontFamily: DESIGN.fonts.body }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#6b6b70', textTransform: 'uppercase', letterSpacing: 0.5, padding: '4px 8px', fontFamily: DESIGN.fonts.body }}>Past Conversations</div>
          {conversations.length === 0 && (
            <div style={{ padding: 16, color: '#8e8e93', fontSize: 13, textAlign: 'center', fontFamily: DESIGN.fonts.body }}>No conversations yet.</div>
          )}
          {conversations.map(c => (
            <button key={c.id} onClick={() => loadConversation(c.id)}
              style={{ display: 'block', width: '100%', textAlign: 'left', padding: 10, border: 'none', background: 'transparent', borderRadius: 8, cursor: 'pointer', fontFamily: DESIGN.fonts.body }}
        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: 16, background: GRY }}>
          {messages.length === 0 && (
            <div style={{ textAlign: 'center', padding: '24px 8px' }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 700, color: BLK, fontFamily: DESIGN.fonts.body }}>
                {clientName ? <>Hi! Let's work on <span style={{ color: T }}>{clientName}</span>.</> : 'Hi! Tell me what you want to do.'}
              </div>
              <div style={{ fontSize: 13, color: '#6b6b70', marginTop: 4, fontFamily: DESIGN.fonts.body }}>I can write briefs, audit pages, build topical maps, find backlinks — just describe it.</div>
              <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {SUGGESTED_PROMPTS.map(p => (
                  <button key={p.label} onClick={() => send(p.label)}
                    style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #ececef', background: '#fff', cursor: 'pointer', textAlign: 'left', fontSize: 13, color: BLK, fontFamily: DESIGN.fonts.body, display: 'flex', alignItems: 'center', gap: 10, transition: 'all .12s' }}
      ...
      {!showHistory && mode !== 'clarifications' && (
        <div style={{ padding: 12, borderTop: '1px solid #ececef', background: '#fff' }}>
          {pendingFile && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: '#f0fdfe', border: `1px solid ${T}`, borderRadius: 8, marginBottom: 8, fontSize: 12, fontFamily: DESIGN.fonts.body, color: BLK, maxWidth: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
            ...
            <textarea
              ...
              rows={1}
              style={{ flex: 1, padding: '10px 12px', border: '1px solid #ececef', borderRadius: 10, fontSize: 14, fontFamily: DESIGN.fonts.body, color: BLK, resize: 'none', outline: 'none', maxHeight: 140, lineHeight: 1.4 }}
      ...
      <div style={{ maxWidth: '80%' }}>
        ...
        {msg.viewActivity && !isUser && onViewActivity && (
          <button onClick={onViewActivity}
            style={{ marginTop: 6, padding: '6px 10px', background: '#fff', color: T, border: `1px solid ${T}`, borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: DESIGN.fonts.body, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
    ...
    return (
      <div style={{ marginTop: 8, border: `1px solid ${T}`, borderRadius: 12, background: '#f0fdfe', padding: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: BLK, fontFamily: DESIGN.fonts.body, textTransform: 'uppercase', letterSpacing: 0.4 }}>
            Pick A Client
          </div>
        </div>
        {executed ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: result?.ok === false ? '#991b1b' : GRN, fontFamily: DESIGN.fonts.body }}>
    ...
    <div style={{ marginTop: 8, border: `1px solid ${T}`, borderRadius: 12, background: '#f0fdfe', padding: 12 }}>
      ...
      {fieldKeys.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 8, padding: 8, marginBottom: 10 }}>
          {fieldKeys.map(k => (
            <div key={k} style={{ display: 'flex', fontSize: 12, padding: '3px 0', fontFamily: DESIGN.fonts.body }}>
      ...
      {showProgress && (
        <div style={{ background: '#fff', borderRadius: 8, padding: 10, marginBottom: 10 }}>
          {progressSteps.map((step, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, fontFamily: DESIGN.fonts.body, padding: '3px 0', color: done ? GRN : active ? BLK : '#8e8e93' }}>
                {done ? (
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {action.should_execute ? (
            <button onClick={() => onRun(idx, true)} disabled={executing}
              style={{ flex: 1, padding: '8px 12px', background: "#0a0a0a", color: '#fff', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: executing ? 'wait' : 'pointer', fontFamily: DESIGN.fonts.body, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              {executing ? <><Loader size={14} className="kotoiq-bot-spin" /> Running…</> : <>Run This</>}
            </button>
          ) : null}
          <button onClick={() => onRun(idx, false)} disabled={executing}
            style={{ flex: action.should_execute ? 'none' : 1, padding: '8px 12px', background: '#fff', color: BLK, border: '1px solid #d1d5db', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: executing ? 'wait' : 'pointer', fontFamily: DESIGN.fonts.body }}>
            {action.should_execute ? 'Modify First' : 'Open Form'}
          </button>
        </div>
      )}
      {action.next_question && !executed && !executing && (
        <div style={{ marginTop: 8, fontSize: 12, color: '#6b6b70', fontStyle: 'italic', fontFamily: DESIGN.fonts.body }}>
    ...
    <div>
      {suggestionRows.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 8, padding: 8, marginBottom: 8, border: `1px dashed ${T}` }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: T, textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 6, fontFamily: DESIGN.fonts.body }}>
            Suggested
          </div>
          {suggestionRows.map(c => (
            <button key={c.id} onClick={() => onPick(c)} type="button"
              style={{ display: 'flex', width: '100%', textAlign: 'left', padding: '8px 10px', border: 'none', borderRadius: 8, background: 'transparent', cursor: 'pointer', gap: 8, alignItems: 'center', fontFamily: DESIGN.fonts.body }}
      ...
      <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #ececef', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', borderBottom: '1px solid #f1f1f6' }}>
          ...
          <input
            ...
            style={{ flex: 1, border: 'none', outline: 'none', fontSize: 13, fontFamily: DESIGN.fonts.body, color: BLK, background: 'transparent' }}
          />
        </div>
        <div style={{ maxHeight: 180, overflowY: 'auto' }}>
          {filtered.length === 0 && (
            <div style={{ padding: 10, fontSize: 12, color: '#8e8e93', fontFamily: DESIGN.fonts.body, textAlign: 'center' }}>
              {safeClients.length === 0 ? 'No clients yet.' : 'No matches.'}
            </div>
          )}
          {filtered.map(c => (
            <button key={c.id} onClick={() => onPick(c)} type="button"
              style={{ display: 'flex', width: '100%', textAlign: 'left', padding: '8px 10px', border: 'none', background: suggestedIds.has(c.id) ? '#f0fdfe' : 'transparent', cursor: 'pointer', gap: 8, alignItems: 'center', fontFamily: DESIGN.fonts.body, borderBottom: '1px solid #f9f9fb' }}
        ...
        <button onClick={onNew} type="button"
          style={{ display: 'flex', width: '100%', alignItems: 'center', gap: 8, padding: '10px 12px', border: 'none', borderTop: '1px solid #ececef', background: '#fafafb', cursor: 'pointer', fontFamily: DESIGN.fonts.body, fontSize: 12, fontWeight: 700, color: T }}
          onMouseEnter={e => e.currentTarget.style.background = '#f0fdfe'}
...
function SystemNote({ text }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'center', margin: '6px 0', animation: 'kotoiqBotFadeIn .25s ease' }}>
      <div style={{
        fontSize: 11, fontFamily: DESIGN.fonts.body, color: '#6b6b70', background: '#eef2ff',
