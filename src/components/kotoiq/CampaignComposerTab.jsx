...
import { FH, BLK, DESIGN } from '../../lib/theme'
          ...
          <div key={s.num} style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{
                ...
                fontSize: 13, fontWeight: 800, fontFamily: DESIGN.fonts.body,
...
export default function CampaignComposerTab({ agencyId }) {
  ...
  return (
    <div>
      <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 4 }}>Campaign Composer</div>
      ...
      {step === 1 && (
        <div style={card}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 16, fontWeight: 700, color: BLK, marginBottom: 16 }}>
            Pick a Template
          </div>
              ...
              {templates.map(tpl => {
                ...
                return (
                  <button key={tpl.id} onClick={() => setSelectedTemplate(tpl)} style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 700, color: BLK }}>{tpl.name || tpl.title || 'Untitled'}</div>
                      {tpl.slot_count != null && (
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 20 }}>
            <button onClick={nextStep} disabled={!selectedTemplate} style={{
              ...
              fontSize: 14, fontWeight: 700, fontFamily: DESIGN.fonts.body,
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20 }}>
            <button onClick={() => setStep(1)} style={{
              ...
              background: '#fff', color: BLK, fontSize: 14, fontWeight: 600, fontFamily: DESIGN.fonts.body,
            ...
            <button onClick={nextStep} style={{
              padding: '10px 24px', borderRadius: 8, border: 'none',
              background: BLK, color: '#fff', fontSize: 14, fontWeight: 700, fontFamily: DESIGN.fonts.body,
              cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6,
            }}>
              Next <ChevronRight size={16} />
            </button>
      ...
      {step === 3 && (
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 16, fontWeight: 700, color: BLK }}>Preview</div>
            ...
            <button onClick={generatePreview} disabled={generatingPreview} style={{
              ...
              background: BLK, color: '#fff', fontSize: 13, fontWeight: 700, fontFamily: DESIGN.fonts.body,
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f9f9fb' }}>
                  <th style={{ padding: '10px 14px', textAlign: 'left', fontFamily: DESIGN.fonts.body, fontWeight: 700, color: BLK, fontSize: 12, borderBottom: '1px solid #ececef' }}>#</th>
                  {seedHeaders.map(h => (
                    <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontFamily: DESIGN.fonts.body, fontWeight: 700, color: BLK, fontSize: 12, borderBottom: '1px solid #ececef' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
        ...
        <div style={card}>
            ...
            {CADENCE_OPTIONS.map(opt => {
              ...
              return (
                <button key={opt.key} onClick={() => setCadence(opt.key)} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 700, color: BLK }}>{opt.label}</div>
          ...
          {cadence === 'drip' && (
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, fontFamily: DESIGN.fonts.body, color: BLK, marginBottom: 6 }}>
                Pages per day
              </label>
              <input type="number" min={1} max={100} value={perDayCap}
                ...
                style={{
                  ...
                  fontSize: 14, fontFamily: DESIGN.fonts.body, fontWeight: 600, width: 120, outline: 'none',
                }}
              />
            </div>
          )}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input type="datetime-local" value={startAt} onChange={e => setStartAt(e.target.value)}
              style={{
                padding: '10px 14px', borderRadius: 8, border: '1px solid #ececef',
                fontSize: 13, fontFamily: DESIGN.fonts.body, width: 240, outline: 'none',
              }}
            />
          </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
            <div style={{ background: '#f9f9fb', borderRadius: 10, padding: '16px 18px' }}>
              ...
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 700, color: BLK }}>{selectedTemplate?.name || selectedTemplate?.title || 'N/A'}</div>
            </div>
            <div style={{ background: '#f9f9fb', borderRadius: 10, padding: '16px 18px' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#6b6b70', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 4 }}>Variants</div>
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 700, color: BLK }}>{seedRows.length} pages</div>
            </div>
            <div style={{ background: '#f9f9fb', borderRadius: 10, padding: '16px 18px' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#6b6b70', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 4 }}>Cadence</div>
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 700, color: BLK }}>
          ...
          {campaignResult && (
            <div style={{
              ...
              <div>
                <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 700, color: '#166534' }}>Campaign Created</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20 }}>
            ...
            {!campaignResult && (
              <button onClick={createCampaign} disabled={creating} style={{
                ...
                color: '#fff', fontSize: 14, fontWeight: 800, fontFamily: DESIGN.fonts.body,
