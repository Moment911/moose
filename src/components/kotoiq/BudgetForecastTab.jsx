  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 800, color: BLK }}>Budget & Forecast</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={generate} disabled={generating}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: "#0a0a0a", color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, fontFamily: DESIGN.fonts.body, cursor: 'pointer', opacity: generating ? 0.6 : 1 }}>
        ...
        <>
          ...
          {forecast?.pacing_status && forecast.pacing_status !== 'on_track' && (
            <div style={{ ...card, background: forecast.pacing_status === 'critical' ? '#fef2f2' : '#fef9c3', borderColor: pacingColor, display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontFamily: DESIGN.fonts.body, fontWeight: 800, color: pacingColor }}>{PACING_LABELS[forecast.pacing_status]}</div>
          ...
          {forecast && (
            <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
              <div style={{ ...card, flex: 1, minWidth: 150 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#6b6b70', textTransform: 'uppercase', fontFamily: DESIGN.fonts.body, marginBottom: 4 }}>Projected {horizon}d Total</div>
                <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 28, fontWeight: 900, color: BLK }}>{fmt(forecast.total_projected)}</div>
              </div>
              <div style={{ ...card, flex: 1, minWidth: 150 }}>
              ...
              <div style={{ ...card, flex: 1, minWidth: 150 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#6b6b70', textTransform: 'uppercase', fontFamily: DESIGN.fonts.body, marginBottom: 4 }}>Pacing</div>
                <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 28, fontWeight: 900, color: pacingColor }}>{PACING_LABELS[forecast.pacing_status] || 'N/A'}</div>
                <div style={{ fontSize: 11, color: '#6b6b70', marginTop: 2 }}>{fmt(p.spent_so_far || 0)} of {fmt(p.budget || 0)} this month</div>
              </div>
              <div style={{ ...card, flex: 1, minWidth: 150 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#6b6b70', textTransform: 'uppercase', fontFamily: DESIGN.fonts.body, marginBottom: 4 }}>Days Until Exhausted</div>
                <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 28, fontWeight: 900, color: p.days_until_exhausted != null && p.days_until_exhausted < 10 ? '#e9695c' : GRN }}>
          ...
          {trend.length > 0 && (
            <div style={card}>
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 800, color: BLK, marginBottom: 16 }}>Daily Spend Trend</div>
          ...
          {forecast && (
            <div style={card}>
              <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 800, color: BLK, marginBottom: 12 }}>Cost Breakdown</div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, fontFamily: DESIGN.fonts.body }}>
                ...
                <tbody>
                  ...
                  <tr style={{ borderTop: '2px solid #ececef' }}>
                    <td style={{ padding: '10px 6px', fontWeight: 800, fontFamily: DESIGN.fonts.body }}>Total</td>
                    <td style={{ padding: '10px 6px', textAlign: 'right', fontWeight: 800 }}>{fmt(forecast.daily_avg_total)}</td>
                    <td style={{ padding: '10px 6px', textAlign: 'right', fontWeight: 800, fontFamily: DESIGN.fonts.body }}>{fmt(forecast.total_projected)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          {!forecast && !loading && (
            <div style={{ ...card, textAlign: 'center', padding: 40, color: '#8e8e93' }}>
              ...
              <div style={{ fontFamily: DESIGN.fonts.body, fontWeight: 700, marginBottom: 4 }}>No forecast data yet</div>
