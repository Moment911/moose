  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
      <div style={{
        ...
        fontFamily: DESIGN.fonts.body, fontSize: isLarge ? 36 : 20, fontWeight: 900, color,
      }}>
        {grade || '—'}
      </div>
      <div style={{ fontFamily: DESIGN.fonts.body, fontSize: isLarge ? 14 : 11, fontWeight: 700, color: '#1f1f22' }}>
  ...
  if (!audit) {
    return (
      <div style={{ ...card, textAlign: 'center', padding: '60px 24px' }}>
        ...
        <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 8 }}>E-E-A-T Scorer</div>
        ...
        {showUrl && (
          <div style={{ maxWidth: 400, margin: '0 auto 16px' }}>
            <input
              ...
              style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1px solid #d1d5db', fontSize: 13, fontFamily: DESIGN.fonts.body, color: BLK, outline: 'none' }}
            />
          </div>
        )}
  ...
  return (
    <>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 20, fontWeight: 800, color: BLK, marginBottom: 4 }}>E-E-A-T Score</div>
              ...
              return (
                <div key={d.key} style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#1f2937', textTransform: 'uppercase', fontFamily: DESIGN.fonts.body }}>{d.key}</span>
                    <span style={{ fontSize: 12, fontWeight: 800, color, fontFamily: DESIGN.fonts.body }}>{d.score}</span>
                  </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <input
          ...
          style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: '1px solid #ececef', fontSize: 12, fontFamily: DESIGN.fonts.body, color: BLK, outline: 'none' }}
        />
        <button onClick={runAudit} disabled={running}
            ...
            <div key={dim.key} style={{ ...card, marginBottom: 0, borderTop: `3px solid ${color}` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, cursor: 'pointer' }} onClick={() => setExpandedDim(expanded ? null : dim.key)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 14, fontWeight: 800, color: BLK }}>{dim.key}</div>
                    <div style={{ fontSize: 12, color: '#1f2937', fontWeight: 600 }}>Weight: {dim.weight}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 24, fontWeight: 900, color }}>{dim.score}</div>
      ...
      {audit.author_name && (
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <div style={{ fontFamily: DESIGN.fonts.body, fontSize: 15, fontWeight: 800, color: BLK }}>Author Entity</div>
          </div>
