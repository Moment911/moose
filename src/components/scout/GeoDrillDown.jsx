...
export default function GeoDrillDown({ onChange, className }) {
  ...
  return (
    <div className={`bg-white rounded-2xl border border-slate-200 overflow-hidden ${className || ''}`} style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      {/* Tabs */}
      <div className="flex border-b border-slate-200 overflow-x-auto">
        {[
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`flex items-center gap-1.5 px-4 py-3 text-xs font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === t.key ? 'border-orange-500 text-orange-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
            {t.label}
      ...
      <div className="max-h-[400px] overflow-auto">
        ...
        {activeTab === 'states' && (
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <button onClick={() => setSelectedStates(US_STATES.map(s => s.code))} className="text-[10px] text-slate-500 hover:text-orange-600 whitespace-nowrap">All</button>
              <button onClick={() => setSelectedStates([])} className="text-[10px] text-slate-500 hover:text-orange-600 whitespace-nowrap">None</button>
            </div>
            {/* Region quick selects */}
            <div className="flex flex-wrap gap-1 mb-3">
              {Object.entries(REGIONS).map(([name, codes]) => (
                <button key={name} onClick={() => selectRegion(codes)} className="text-[10px] px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 hover:bg-orange-50 hover:text-orange-600 transition-colors">{name}</button>
              ))}
              <button onClick={() => setSelectedStates(US_STATES.map(s => s.code))} className="text-[10px] px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 hover:bg-orange-50 hover:text-orange-600 transition-colors">All US</button>
            </div>
            {/* Selected pills */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1">
              {filteredStates.map(s => (
                <button key={s.code} onClick={() => toggleState(s.code)}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-left transition-colors ${selectedStates.includes(s.code) ? 'bg-orange-50 text-orange-700 font-medium' : 'text-slate-600 hover:bg-slate-50'}`}>
                  ...
                  {s.name} <span className="text-slate-400 ml-auto">{s.code}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'counties' && (
          <div className="p-4">
            {selectedStates.length === 0 ? (
              <div className="text-center py-8 text-sm text-slate-400"><MapPin size={24} className="mx-auto mb-2 text-slate-300" />Select state(s) first</div>
            ) : (
              <>
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex-1 flex items-center gap-2 border border-slate-200 rounded-lg px-3 py-1.5">
                  ...
                  <button onClick={selectAllCounties} className={`text-[10px] whitespace-nowrap px-2 py-0.5 rounded ${selectedCounties.length === filteredCounties.length && filteredCounties.length > 0 ? 'bg-orange-500 text-white' : 'text-slate-500 hover:text-orange-600'}`}>
                  ...
                  <button onClick={deselectAllCounties} className="text-[10px] text-slate-500 hover:text-orange-600 whitespace-nowrap">Clear</button>
                </div>
                {loadingCounties ? (
                  <div className="flex items-center justify-center py-8 gap-2 text-sm text-slate-400"><Loader2 size={16} className="animate-spin" /> Loading counties...</div>
                ) : (
                  <div className="space-y-0.5 max-h-[280px] overflow-auto">
                    {filteredCounties.map(c => (
                      <button key={`${c.stateFips}-${c.fips}`} onClick={() => toggleCounty(c)}
                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-left transition-colors ${selectedCounties.some(sc => sc.fips === c.fips && sc.stateFips === c.stateFips) ? 'bg-orange-50 text-orange-700 font-medium' : 'text-slate-600 hover:bg-slate-50'}`}>
                        ...
                        <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">{c.stateCode}</span>
                      </button>
                    ))}
                    {filteredCounties.length === 0 && <p className="text-center py-4 text-sm text-slate-400">No counties found</p>}
                  </div>
                )}
              </>
            )}
        ...
        {activeTab === 'cities' && (
          <div className="p-4">
            {selectedStates.length === 0 ? (
              <div className="text-center py-8 text-sm text-slate-400"><Building2 size={24} className="mx-auto mb-2 text-slate-300" />Select state(s) first</div>
            ) : (
              <>
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex-1 flex items-center gap-2 border border-slate-200 rounded-lg px-3 py-1.5">
                  ...
                  <button onClick={selectAllCities} className={`text-[10px] whitespace-nowrap px-2 py-0.5 rounded ${selectedCities.length === filteredCitiesResult.length && filteredCitiesResult.length > 0 ? 'bg-orange-500 text-white' : 'text-slate-500 hover:text-orange-600'}`}>
                  ...
                  <button onClick={deselectAllCities} className="text-[10px] text-slate-500 hover:text-orange-600 whitespace-nowrap">Clear</button>
                </div>
                {loadingCities ? (
                  <div className="flex items-center justify-center py-8 gap-2 text-sm text-slate-400"><Loader2 size={16} className="animate-spin" /> Loading cities...</div>
                ) : (
                  <div className="space-y-0.5 max-h-[280px] overflow-auto">
                    {filteredCitiesResult.map(c => (
                      <button key={`${c.stateFips}-${c.fips}`} onClick={() => toggleCity(c)}
                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-left transition-colors ${selectedCities.some(sc => sc.fips === c.fips && sc.stateFips === c.stateFips) ? 'bg-orange-50 text-orange-700 font-medium' : 'text-slate-600 hover:bg-slate-50'}`}>
                        ...
                        <span className="flex-1">{c.name}</span>
                        <span className="text-[10px] text-slate-400">{formatPop(c.population)}</span>
                        <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">{c.stateCode}</span>
                      </button>
                    ))}
                    {filteredCitiesResult.length === 0 && <p className="text-center py-4 text-sm text-slate-400">No cities found</p>}
                  </div>
                )}
              </>
            )}
        ...
        {activeTab === 'zips' && (
          <div className="p-4 space-y-4">
            {/* Manual ZIP entry */}
            <div>
              <p className="text-xs font-semibold text-slate-700 mb-2">Add ZIP Codes</p>
              <div className="flex gap-2">
                <textarea className="flex-1 text-sm border border-slate-200 rounded-lg px-3 py-2 resize-none" rows={2}
              ...
              {zipCodes.length > 0 && (
                <div className="flex items-center gap-2 mt-2 mb-1">
                  <span className="text-[10px] text-slate-500 font-medium">{zipCodes.length} zip codes</span>
                  <button onClick={copyAllZips} className="text-[10px] text-orange-500 hover:text-orange-700">📋 Copy All</button>
                  <button onClick={() => setZipCodes([])} className="text-[10px] text-slate-400 hover:text-red-500">Clear</button>
                </div>
              )}
              {zipCodes.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {zipCodes.slice(0, 30).map(z => (
                    <span key={z} className="inline-flex items-center gap-1 bg-slate-100 text-slate-600 text-[10px] px-2 py-0.5 rounded-full">
                      {z} <button onClick={() => setZipCodes(prev => prev.filter(x => x !== z))} className="hover:text-red-500"><X size={8} /></button>
                    </span>
                  ))}
                  {zipCodes.length > 30 && <span className="text-[10px] text-slate-400">+{zipCodes.length - 30} more</span>}
                </div>
              )}
            </div>
            ...
            {/* Radius */}
            <div>
              <p className="text-xs font-semibold text-slate-700 mb-2">Radius Search</p>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-slate-500">Radius: <strong>{radiusMiles} miles</strong></span>
                </div>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {[5, 10, 15, 25, 50, 100].map(r => (
                    <button key={r} onClick={() => setRadiusMiles(r)}
                      className={`text-[10px] px-2.5 py-1 rounded-lg font-medium transition-colors ${radiusMiles === r ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{r} mi</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        {activeTab === 'metros' && (
          <div className="p-4">
            <p className="text-xs text-slate-500 mb-3">Quick-select major metropolitan areas</p>
            <div className="grid grid-cols-2 gap-2">
              {METRO_AREAS.map(m => (
                <button key={m.id} onClick={() => selectMetro(m)}
                  className="text-left p-3 rounded-xl border border-slate-200 hover:border-orange-300 hover:bg-orange-50 transition-all group">
                  <p className="text-sm font-medium text-slate-800 group-hover:text-orange-700">{m.name}</p>
                  <p className="text-[10px] text-slate-400">{m.counties.length} counties &middot; {formatPop(m.pop)} pop</p>
                </button>
              ))}
            </div>
          </div>
      ...
      {totalSelections > 0 && (
        <div className="px-4 py-3 bg-slate-50 border-t border-slate-200 flex items-center gap-3">
          ...
          <button onClick={clearAll} className="text-[10px] text-slate-400 hover:text-red-500">Clear all</button>
        </div>
      )}
    </div>
  )
