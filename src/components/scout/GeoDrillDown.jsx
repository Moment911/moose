    ...
    <div className={`bg-white rounded-2xl border border-zinc-200 overflow-hidden ${className || ''}`} style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      <div className="flex border-b border-zinc-200 overflow-x-auto">
        {[
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`flex items-center gap-1.5 px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === t.key ? 'border-brand-500 text-brand-600' : 'border-transparent text-zinc-500 hover:text-zinc-700'}`}>
            {t.label}
            {t.badge > 0 && <span className="bg-brand-500 text-white text-[13px] px-1.5 py-0.5 rounded-full min-w-[18px] text-center">{t.badge}</span>}
          </button>
        ))}
      </div>
      ...
      <div className="max-h-[400px] overflow-auto">
        ...
        {activeTab === 'states' && (
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <button onClick={() => setSelectedStates(US_STATES.map(s => s.code))} className="text-[13px] text-zinc-500 hover:text-brand-600 whitespace-nowrap">All</button>
              <button onClick={() => setSelectedStates([])} className="text-[13px] text-zinc-500 hover:text-brand-600 whitespace-nowrap">None</button>
            </div>
            {/* Region quick selects */}
            <div className="flex flex-wrap gap-1 mb-3">
              {Object.entries(REGIONS).map(([name, codes]) => (
                <button key={name} onClick={() => selectRegion(codes)} className="text-[13px] px-2.5 py-1 rounded-lg bg-zinc-100 text-zinc-600 hover:bg-red-50 hover:text-brand-600 transition-colors">{name}</button>
              ))}
              <button onClick={() => setSelectedStates(US_STATES.map(s => s.code))} className="text-[13px] px-2.5 py-1 rounded-lg bg-zinc-100 text-zinc-600 hover:bg-red-50 hover:text-brand-600 transition-colors">All US</button>
            </div>
            {/* Selected pills */}
            {selectedStates.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {selectedStates.map(code => (
                  <span key={code} className="inline-flex items-center gap-1 bg-red-50 text-red-700 text-[13px] font-medium px-2 py-0.5 rounded-full">
                    {code} <button onClick={() => toggleState(code)} className="hover:text-red-900"><X size={9} /></button>
                  </span>
                ))}
              </div>
            )}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1">
              {filteredStates.map(s => (
                <button key={s.code} onClick={() => toggleState(s.code)}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition-colors ${selectedStates.includes(s.code) ? 'bg-red-50 text-red-700 font-medium' : 'text-zinc-600 hover:bg-zinc-50'}`}>
                  <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${selectedStates.includes(s.code) ? 'bg-brand-500 border-brand-500' : 'border-slate-300'}`}>
            ...
            {selectedStates.length === 0 ? (
              ...
              <>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <button onClick={selectAllCounties} className={`text-[13px] whitespace-nowrap px-2 py-0.5 rounded ${selectedCounties.length === filteredCounties.length && filteredCounties.length > 0 ? 'bg-brand-500 text-white' : 'text-zinc-500 hover:text-brand-600'}`}>
                  ...
                  <button onClick={deselectAllCounties} className="text-[13px] text-zinc-500 hover:text-brand-600 whitespace-nowrap">Clear</button>
                </div>
                {loadingCounties ? (
                  ...
                  <div className="space-y-0.5 max-h-[280px] overflow-auto">
                    {filteredCounties.map(c => (
                      <button key={`${c.stateCode}-${c.name}`} onClick={() => toggleCounty(c)}
                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition-colors ${selectedCounties.some(sc => sc.name === c.name && sc.stateCode === c.stateCode) ? 'bg-red-50 text-red-700 font-medium' : 'text-zinc-600 hover:bg-zinc-50'}`}>
                        <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${selectedCounties.some(sc => sc.name === c.name && sc.stateCode === c.stateCode) ? 'bg-brand-500 border-brand-500' : 'border-slate-300'}`}>
                          {selectedCounties.some(sc => sc.name === c.name && sc.stateCode === c.stateCode) && <Check size={10} className="text-white" />}
                        </div>
                        <span className="flex-1">{c.name}</span>
                        <span className="text-[13px] bg-zinc-100 text-zinc-500 px-1.5 py-0.5 rounded">{c.stateCode}</span>
                      </button>
                    ))}
            ...
            {selectedStates.length === 0 ? (
              ...
              <>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <button onClick={selectAllCities} className={`text-[13px] whitespace-nowrap px-2 py-0.5 rounded ${selectedCities.length === filteredCitiesResult.length && filteredCitiesResult.length > 0 ? 'bg-brand-500 text-white' : 'text-zinc-500 hover:text-brand-600'}`}>
                  ...
                  <button onClick={deselectAllCities} className="text-[13px] text-zinc-500 hover:text-brand-600 whitespace-nowrap">Clear</button>
                </div>
                {loadingCities ? (
                  ...
                  <div className="space-y-0.5 max-h-[280px] overflow-auto">
                    {filteredCitiesResult.map(c => (
                      <button key={`${c.stateCode}-${c.name}`} onClick={() => toggleCity(c)}
                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition-colors ${selectedCities.some(sc => sc.name === c.name && sc.stateCode === c.stateCode) ? 'bg-red-50 text-red-700 font-medium' : 'text-zinc-600 hover:bg-zinc-50'}`}>
                        <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${selectedCities.some(sc => sc.name === c.name && sc.stateCode === c.stateCode) ? 'bg-brand-500 border-brand-500' : 'border-slate-300'}`}>
        ...
        {activeTab === 'zips' && (
          <div className="p-4 space-y-4">
            {/* Manual ZIP entry */}
            <div>
              <p className="text-sm font-semibold text-zinc-700 mb-2">Add ZIP Codes</p>
              <div className="flex gap-2">
                ...
                <button onClick={addZipsFromInput} className="bg-brand-500 hover:bg-brand-600 text-white text-sm font-semibold px-4 rounded-lg transition-colors self-end">Add</button>
              </div>
              {zipCodes.length > 0 && (
                <div className="flex items-center gap-2 mt-2 mb-1">
                  <span className="text-[13px] text-zinc-500 font-medium">{zipCodes.length} zip codes</span>
                  <button onClick={copyAllZips} className="text-[13px] text-brand-500 hover:text-red-700">📋 Copy All</button>
                  <button onClick={() => setZipCodes([])} className="text-[13px] text-zinc-400 hover:text-red-500">Clear</button>
                </div>
              )}
              {zipCodes.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {zipCodes.slice(0, 30).map(z => (
                    <span key={z} className="inline-flex items-center gap-1 bg-zinc-100 text-zinc-600 text-[13px] px-2 py-0.5 rounded-full">
                      {z} <button onClick={() => setZipCodes(prev => prev.filter(x => x !== z))} className="hover:text-red-500"><X size={8} /></button>
                    </span>
                  ))}
                  {zipCodes.length > 30 && <span className="text-[13px] text-zinc-400">+{zipCodes.length - 30} more</span>}
                </div>
              )}
            </div>
            ...
            {/* Radius */}
            <div>
              <p className="text-sm font-semibold text-zinc-700 mb-2">Radius Search</p>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm text-zinc-500">Radius: <strong>{radiusMiles} miles</strong></span>
                </div>
                <input type="range" min={1} max={100} value={radiusMiles} onChange={e => setRadiusMiles(+e.target.value)} className="w-full accent-brand-500" />
                <div className="flex flex-wrap gap-1 mt-2">
                  {[5, 10, 15, 25, 50, 100].map(r => (
                    <button key={r} onClick={() => setRadiusMiles(r)}
                      className={`text-[13px] px-2.5 py-1 rounded-lg font-medium transition-colors ${radiusMiles === r ? 'bg-brand-500 text-white' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}>{r} mi</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        {activeTab === 'metros' && (
          <div className="p-4">
            <p className="text-sm text-zinc-500 mb-3">Quick-select major metropolitan areas</p>
            <div className="grid grid-cols-2 gap-2">
              {METRO_AREAS.map(m => (
                <button key={m.id} onClick={() => selectMetro(m)}
                  className="text-left p-3 rounded-xl border border-zinc-200 hover:border-orange-300 hover:bg-red-50 transition-all group">
                  <p className="text-sm font-medium text-zinc-800 group-hover:text-red-700">{m.name}</p>
                  <p className="text-[13px] text-zinc-400">{m.counties.length} counties &middot; {formatPop(m.pop)} pop</p>
                </button>
              ))}
            </div>
          </div>
      ...
      {totalSelections > 0 && (
        <div className="px-4 py-3 bg-zinc-50 border-t border-zinc-200 flex items-center gap-3">
          ...
          <button onClick={clearAll} className="text-[13px] text-zinc-400 hover:text-red-500">Clear all</button>
        </div>
      )}
    </div>
  )
