...
export default function MobileHeader({ title, onMenuPress, onBack, backLabel, rightAction, showLogo = false }) {
  return (
    <header className="md:hidden" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 40, background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: '0.5px solid #e5e7eb', paddingTop: 'env(safe-area-inset-top, 0px)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 44, padding: '0 12px' }}>
        ...
        <div style={{ width: 80, display: 'flex', alignItems: 'center' }}>
          {onBack ? (
            ...
            <button onClick={onMenuPress} style={{ background: 'none', border: 'none', padding: 8, cursor: 'pointer', marginLeft: -8, WebkitTapHighlightColor: 'transparent' }}>
              <Menu size={22} strokeWidth={1.8} color="#374151" />
            </button>
          ) : <div style={{ width: 32 }} />}
        </div>
        {/* Center */}
        <div style={{ flex: 1, textAlign: 'center' }}>
          {showLogo
            ? <LucyLogo size="sm" />
            : <span style={{ fontSize: 17, fontWeight: 600, color: '#111', letterSpacing: -0.2 }}>{title}</span>}
        </div>
        {/* Right */}
