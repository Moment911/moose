                  ...
                  {(result.blocks || []).map((b, i) => {
                    ...
                    return (
                      <div key={i}>
                        ...
                        {b.type === 'button' && <div style={{ background: d.bgColor || '#fff', padding: `${d.padding || 24}px 32px`, textAlign: d.align || 'center' }}><span style={{ display: 'inline-block', background: d.btnColor || '#E6007E', color: d.textColor || '#fff', padding: '12px 28px', borderRadius: d.borderRadius || 8, fontWeight: 600, fontSize: 14 }}>{d.text}</span></div>}
                        {b.type === 'hero' && <div style={{ background: d.bgColor || '#231f20', padding: '40px 32px', textAlign: 'center' }}><h1 style={{ margin: 0, color: d.textColor || '#fff', fontSize: 28, fontWeight: 800 }}>{d.heading}</h1>{d.subtext && <p style={{ margin: '12px 0', color: d.textColor || '#fff', opacity: 0.8, fontSize: 14 }}>{d.subtext}</p>}{d.btnText && <span style={{ display: 'inline-block', background: d.btnColor || '#E6007E', color: '#fff', padding: '12px 28px', borderRadius: 8, fontWeight: 600, fontSize: 14, marginTop: 8 }}>{d.btnText}</span>}</div>}
                        {b.type === 'quote' && <div style={{ background: d.bgColor || '#f9fafb', padding: `${d.padding || 24}px 32px` }}><div style={{ borderLeft: '4px solid #E6007E', paddingLeft: 16 }}><p style={{ margin: 0, fontStyle: 'italic', color: d.textColor || '#374151', fontSize: 16 }}>{d.text}</p>{d.author && <p style={{ margin: '8px 0 0', color: d.textColor || '#374151', opacity: 0.6, fontSize: 13 }}>&mdash; {d.author}</p>}</div></div>}
