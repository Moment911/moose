-- ══════════════════════════════════════════════════════════════════════════════
-- KOTO ANSWERING — CALL CENTER UPGRADE
-- Additive migration. Safe to run multiple times. Run in Supabase SQL Editor.
-- ══════════════════════════════════════════════════════════════════════════════
...
ALTER TABLE koto_inbound_agents ADD COLUMN IF NOT EXISTS digest_schedule   text;         -- e.g. 'weekly', 'daily'
ALTER TABLE koto_inbound_agents ADD COLUMN IF NOT EXISTS hipaa_mode        boolean DEFAULT false;
ALTER TABLE koto_inbound_agents ADD COLUMN IF NOT EXISTS retention_days    int;          -- null = keep forever
...
-- ── Call record — new fields the webhook populates ─────────────────────────
...
-- ── Helpful indexes for the analytics + follow-up queries ──────────────────
