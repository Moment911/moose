  ...
  function renderBlock(b) {
    ...
    return (
      <div key={b.id} className={`relative group cursor-pointer transition-all ${isSel ? 'ring-2 ring-brand-500 ring-offset-2' : 'hover:ring-1 hover:ring-gray-300 hover:ring-offset-1'}`}
        ...
        {/* Render */}
        {b.type === 'header' && <div style={{ background: d.bgColor, padding: '32px 40px', textAlign: 'center' }}><h1 style={{ margin: 0, color: d.textColor, fontSize: 28, fontWeight: 700 }}>{d.logoText}</h1>{d.tagline && <p style={{ margin: '8px 0 0', color: d.textColor, opacity: 0.7, fontSize: 14 }}>{d.tagline}</p>}</div>}
        ...
        {b.type === 'quote' && <div style={{ background: d.bgColor, padding: `${d.padding}px 40px` }}><div style={{ borderLeft: '4px solid #ea2729', paddingLeft: 20 }}><p style={{ margin: 0, color: d.textColor, fontSize: 18, fontStyle: 'italic', lineHeight: 1.6, fontFamily: 'Georgia, serif' }}>{d.text}</p>{d.author && <p style={{ margin: '12px 0 0', color: d.textColor, opacity: 0.6, fontSize: 14 }}>&mdash; {d.author}</p>}</div></div>}
        ...
        {b.type === 'twocol' && <div style={{ background: d.bgColor, padding: `${d.padding}px 40px`, display: 'flex', gap: 24 }}><div style={{ flex: 1, color: d.textColor, fontSize: 14, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{d.leftContent}</div><div style={{ flex: 1, color: d.textColor, fontSize: 14, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{d.rightContent}</div></div>}
        {b.type === 'footer' && <div style={{ background: d.bgColor, padding: `${d.padding}px 40px`, textAlign: 'center' }}><p style={{ margin: 0, color: d.textColor, fontSize: 12, lineHeight: 1.6 }}>{d.text}</p>{d.links && <p style={{ margin: '8px 0 0', color: d.textColor, fontSize: 11 }}>{d.links}</p>}</div>}
      </div>
    )
  }
        ...
        <div className="h-14 bg-white border-b border-gray-200 px-6 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 ml-4">
            {[{ n: 1, l: 'Setup' }, { n: 2, l: 'Design' }, { n: 3, l: 'Send' }].map(s => (
              <button key={s.n} onClick={() => setStep(s.n)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${step === s.n ? 'bg-brand-500 text-white' : step > s.n ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                {step > s.n && <Check size={11} />}{s.l}
              </button>
            ))}
          </div>
          <div className="ml-auto flex gap-2">
            <button onClick={saveDraft} className="btn-secondary text-xs">Save Draft</button>
            {step < 3 ? <button onClick={() => setStep(step + 1)} className="btn-primary text-xs">Next</button> : null}
          </div>
        </div>
              ...
              <div className="max-w-3xl mx-auto grid grid-cols-2 gap-6">
                <div className="bg-white rounded-2xl p-6 border border-gray-100" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                  ...
                  <div className="space-y-3">
                    <div><label className="text-xs text-gray-500 block mb-1">Campaign Name</label><input className="input text-sm" placeholder="Monthly Newsletter" value={campaign.name} onChange={e => setCampaign(c => ({ ...c, name: e.target.value }))} /></div>
                    <div>
                      <div className="flex items-center justify-between mb-1"><label className="text-xs text-gray-500">Subject Line</label><button onClick={generateSubjects} disabled={generatingSubjects} className="text-[10px] text-brand-500 hover:text-brand-700 flex items-center gap-1 disabled:opacity-50">{generatingSubjects ? <Loader2 size={10} className="animate-spin" /> : <Wand2 size={10} />} {generatingSubjects ? 'Generating...' : 'AI Suggest'}</button></div>
                      ...
                      {showAiSubjects && <div className="mt-2 bg-gray-50 rounded-lg p-2 space-y-1">
                        {generatingSubjects && <div className="flex items-center gap-2 px-2 py-2 text-xs text-gray-400"><Loader2 size={12} className="animate-spin" /> Generating subject lines...</div>}
                        {aiSubjects.map((s, i) => <button key={i} onClick={() => { setCampaign(c => ({ ...c, subject: s })); setShowAiSubjects(false) }} className="w-full text-left text-xs text-gray-700 hover:bg-white rounded px-2 py-1.5">{s}</button>)}
                        {!generatingSubjects && aiSubjects.length > 0 && <button onClick={() => setShowAiSubjects(false)} className="w-full text-center text-[10px] text-gray-400 hover:text-gray-600 pt-1">Close</button>}
                      </div>}
                    </div>
                    <div><label className="text-xs text-gray-500 block mb-1">Preview Text</label><input className="input text-sm" value={campaign.preview_text} onChange={e => setCampaign(c => ({ ...c, preview_text: e.target.value }))} /></div>
                    <div><label className="text-xs text-gray-500 block mb-1">From Name</label><input className="input text-sm" value={campaign.from_name} onChange={e => setCampaign(c => ({ ...c, from_name: e.target.value }))} /></div>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-6 border border-gray-100" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          ...
          {step === 2 && (
            <>
              ...
              <div className="w-48 bg-white border-r border-gray-200 overflow-y-auto flex-shrink-0 p-3">
                <button onClick={() => setShowAIAssistant(true)} className="w-full flex items-center gap-2 px-3 py-2.5 mb-1.5 text-xs font-medium text-white bg-brand-500 hover:bg-brand-600 rounded-xl transition-colors"><Sparkles size={13} /> Design with AI</button>
                <button onClick={() => setShowTemplates(true)} className="w-full flex items-center gap-2 px-3 py-2 mb-1.5 text-xs font-medium text-brand-600 bg-brand-50 hover:bg-brand-100 rounded-xl transition-colors"><Layout size={13} /> Templates</button>
                <button onClick={() => setShowImageFinder(true)} className="w-full flex items-center gap-2 px-3 py-2 mb-2 text-xs font-medium text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors"><Search size={13} /> Find Images</button>
                <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Add Block</p>
                {BLOCK_TYPES.map(bt => { const I = bt.icon; return (
                  <button key={bt.type} onClick={() => addBlock(bt.type)} className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                ...
                {brandAssets.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-gray-100">
                    <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-1"><Palette size={9} /> Brand Assets</p>
                    {brandAssets.map((ba, bi) => (
                      <div key={bi} className="mb-3">
                        <p className="text-[10px] font-medium text-gray-600 mb-1.5">{ba.clients?.name || 'Client'}</p>
                        {/* Colors */}
                        {(ba.primary_colors || []).length > 0 && (
                          <div className="mb-2">
                            <p className="text-[9px] text-gray-400 mb-1">Colors</p>
                        ...
                        {(ba.fonts || []).length > 0 && (
                          <div className="mb-2">
                            <p className="text-[9px] text-gray-400 mb-1">Fonts</p>
                            {(ba.fonts || []).map((f, fi) => (
                              <p key={fi} className="text-[10px] text-gray-600 cursor-pointer hover:text-brand-600" onClick={() => { navigator.clipboard.writeText(f.name); toast.success(`${f.name} copied`) }} title="Click to copy font name">{f.name} <span className="text-gray-300">({f.category})</span></p>
                            ))}
                          </div>
                        )}
                        {/* Logos */}
                        {(ba.logo_files || []).length > 0 && (
                          <div>
                            <p className="text-[9px] text-gray-400 mb-1">Logos</p>
                            <div className="flex flex-wrap gap-1">
                ...
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <p className="text-[9px] text-gray-400">Click a block to add it. Click any block in the email to edit its properties.</p>
                </div>
              </div>
              ...
              {sel ? (
                <div className="w-64 bg-white border-l border-gray-200 overflow-y-auto flex-shrink-0 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[10px] font-semibold text-gray-500 uppercase">{BLOCK_TYPES.find(t => t.type === sel.type)?.label}</p>
                  ...
                  <div className="space-y-3">
                    ...
                    {sel.type === 'text' && (<>
                      <div><label className="text-[10px] text-gray-500 mb-1 block">Content</label><textarea className="input text-xs py-1 resize-none" rows={5} value={sel.data.content} onChange={e => updateBlock(sel.id, { content: e.target.value })} /></div>
                    ...
                    {sel.type === 'image' && (<>
                      <button onClick={() => fileInputRef.current?.click()} className="w-full btn-secondary text-xs justify-center"><ImageIcon size={12} /> {sel.data.src ? 'Replace' : 'Upload'}</button>
                      <button onClick={() => setShowImageFinder(true)} className="w-full btn-secondary text-xs justify-center"><Search size={12} /> Find Stock Image</button>
                    ...
                    {sel.type === 'twocol' && (<>
                      <div><label className="text-[10px] text-gray-500 mb-1 block">Left</label><textarea className="input text-xs py-1 resize-none" rows={3} value={sel.data.leftContent} onChange={e => updateBlock(sel.id, { leftContent: e.target.value })} /></div>
                      <div><label className="text-[10px] text-gray-500 mb-1 block">Right</label><textarea className="input text-xs py-1 resize-none" rows={3} value={sel.data.rightContent} onChange={e => updateBlock(sel.id, { rightContent: e.target.value })} /></div>
                      <ColorPicker label="Text" value={sel.data.textColor} onChange={v => updateBlock(sel.id, { textColor: v })} />
                    </>)}
                    {sel.type === 'heading' && (<>
                    ...
                    {sel.type === 'quote' && (<>
                      <div><label className="text-[10px] text-gray-500 mb-1 block">Quote</label><textarea className="input text-xs py-1 resize-none" rows={3} value={sel.data.text} onChange={e => updateBlock(sel.id, { text: e.target.value })} /></div>
                    ...
                    {sel.type === 'list' && (<>
                      <div>
                        <label className="text-[10px] text-gray-500 mb-1 block">Items (one per line)</label>
                        <textarea className="input text-xs py-1 resize-none" rows={5} value={(sel.data.items || []).join('\n')} onChange={e => updateBlock(sel.id, { items: e.target.value.split('\n') })} />
                      </div>
                      <ColorPicker label="Text Color" value={sel.data.textColor} onChange={v => updateBlock(sel.id, { textColor: v })} />
                      <ColorPicker label="Background" value={sel.data.bgColor} onChange={v => updateBlock(sel.id, { bgColor: v })} />
                    </>)}
                    {sel.type === 'hero' && (<>
                      ...
                      <div><label className="text-[10px] text-gray-500 mb-1 block">Subtext</label><textarea className="input text-xs py-1 resize-none" rows={3} value={sel.data.subtext} onChange={e => updateBlock(sel.id, { subtext: e.target.value })} /></div>
                    ...
                    {brandAssets.some(ba => ba.primary_colors?.length) && (
                      <div className="border-t border-gray-100 pt-2 mt-1">
                        <p className="text-[9px] text-gray-400 mb-1.5">Brand Colors</p>
                        <div className="flex flex-wrap gap-1">
                    ...
                    <button onClick={() => deleteBlock(sel.id)} className="w-full text-xs text-brand-500 hover:bg-brand-50 py-2 rounded-lg flex items-center justify-center gap-1"><Trash2 size={11} /> Delete</button>
                  </div>
                </div>
              ) : showPersonalization ? (
                ...
                <div className="w-64 bg-white border-l border-gray-200 p-4 flex-shrink-0">
                  ...
                  <div className="border-t border-gray-100 pt-3 mt-3">
                    ...
                    <p className="text-[10px] text-gray-400 mb-2">Applied to all recipients on send</p>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowTemplates(false)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div><h3 className="font-semibold text-gray-900">Email Templates</h3><p className="text-xs text-gray-500 mt-0.5">Choose a starting point for your email</p></div>
                        ...
                        {tpl.blocks.length > 0 ? (
                          <div className="transform scale-[0.25] origin-top-left" style={{ width: 600, pointerEvents: 'none' }}>
                            {tpl.blocks.map((b, bi) => {
                              ...
                              if (b.type === 'text') return <div key={bi} style={{ background: d.bgColor, padding: '16px 40px' }}><p style={{ margin: 0, color: d.textColor, fontSize: 14, lineHeight: 1.4 }}>{(d.content || '').slice(0, 60)}...</p></div>
                              if (b.type === 'button') return <div key={bi} style={{ background: d.bgColor, padding: '12px 40px', textAlign: d.align }}><span style={{ display: 'inline-block', background: d.btnColor, color: d.textColor, padding: '10px 24px', borderRadius: d.borderRadius, fontSize: 14, fontWeight: 600 }}>{d.text}</span></div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                ...
                <div className="flex items-center gap-2">
                  {['desktop', 'mobile'].map(m => <button key={m} onClick={() => setPreviewMode(m)} className={`text-xs px-3 py-1 rounded-lg capitalize ${previewMode === m ? 'bg-gray-200 text-gray-800' : 'text-gray-500'}`}>{m}</button>)}
...
function Field({ label, value, onChange }) {
  return <div><label className="text-[10px] text-gray-500 mb-1 block">{label}</label><input className="input text-xs py-1" value={value || ''} onChange={e => onChange(e.target.value)} /></div>
}
function NumField({ label, value, onChange, min = 0, max = 999 }) {
  return <div><label className="text-[10px] text-gray-500 mb-1 block">{label}</label><input className="input text-xs py-1" type="number" min={min} max={max} value={value} onChange={e => onChange(+e.target.value)} /></div>
}
function SelField({ label, value, options, onChange }) {
  return <div><label className="text-[10px] text-gray-500 mb-1 block">{label}</label><select className="input text-xs py-1" value={value} onChange={e => onChange(e.target.value)}>{options.map(o => <option key={o}>{o}</option>)}</select></div>
}
