      ...
      <main className="flex-1 overflow-y-auto" style={{ background: '#F8F9FC' }}>
        <div className="px-4 md:px-8 py-4 md:py-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-8" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
            <div className="grid grid-cols-[1fr_80px_80px_80px_100px] gap-4 px-5 py-3 bg-gray-50/50 text-[10px] font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">
            ...
            {campaigns.map(c => (
              <div key={c.id} className="grid grid-cols-[1fr_80px_80px_80px_100px] gap-4 px-5 py-3.5 items-center border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer" onClick={() => navigate(`/marketing/campaigns/${c.id}`)}>
                <div><p className="text-sm font-medium text-gray-900">{c.name}</p><p className="text-xs text-gray-400">{c.subject}</p></div>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${c.status === 'sent' ? 'bg-green-50 text-green-700' : c.status === 'draft' ? 'bg-gray-100 text-gray-600' : 'bg-blue-50 text-blue-700'}`}>{c.status}</span>
