      ...
      <main className="flex-1 overflow-y-auto bg-white">
        ...
        <div className="px-4 md:px-8 py-4 md:py-6">
          ...
          {showAdd && (
            <div className="card p-5 mb-4">
              <form onSubmit={handleAdd} className="grid grid-cols-4 gap-3 items-end">
                ...
                <div><label className="text-xs text-gray-500 block mb-1">Project</label>
                ...
                <div><label className="text-xs text-gray-500 block mb-1">Invoice #</label>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden" style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
            <div className="grid grid-cols-[1fr_100px_80px_90px_80px_80px_60px] gap-3 px-4 py-2.5 bg-gray-50 text-[10px] font-semibold text-gray-500 uppercase tracking-wider border-b" style={{ minWidth: 640 }}>
            ...
            {filtered.map(r => (
              <div key={r.id} className="grid grid-cols-[1fr_100px_80px_90px_80px_80px_60px] gap-3 px-4 py-3 items-center border-b border-gray-100 hover:bg-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{r.description || r.projects?.name || ''}</p>
                </div>
                <span className="text-sm font-semibold text-gray-900">${parseFloat(r.amount || 0).toLocaleString()}</span>
                <span className="text-xs text-gray-500 capitalize">{r.type?.replace('_', ' ')}</span>
                <span className="text-xs text-gray-400">{r.invoice_number || '—'}</span>
                <span className="text-xs text-gray-400">{r.due_date ? format(new Date(r.due_date), 'MMM d') : '—'}</span>
                <div>
                  {r.status === 'paid' ? (
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-green-50 text-green-700">Paid</span>
                  ) : (
                    <button onClick={() => markPaid(r.id)} className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-amber-50 text-amber-700 hover:bg-green-50 hover:text-green-700 transition-colors cursor-pointer">Pending</button>
                  )}
                </div>
          ...
          <p className="text-xs text-gray-400 mt-3">{filtered.length} records</p>
        </div>
      </main>
    </div>
  )
