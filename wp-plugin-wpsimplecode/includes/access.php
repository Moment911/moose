...
add_action('rest_api_init', function () {
    ...
    register_rest_route(WPSC_REST_NS, '/access/apply', [
        'methods'  => 'POST',
        'callback' => 'wpsc_am_apply',
        'permission_callback' => 'wpsc_perm_write',
    ]);
    register_rest_route(WPSC_REST_NS, '/access/snapshot', [
        'methods'  => 'POST',
        'callback' => 'wpsc_am_snapshot',
        'permission_callback' => 'wpsc_perm_write',
    ]);
    register_rest_route(WPSC_REST_NS, '/access/revert', [
        'methods'  => 'POST',
        'callback' => 'wpsc_am_revert',
        'permission_callback' => 'wpsc_perm_write',
    ]);
});

function wpsc_am_roles($req) {
    ...
    return rest_ensure_response([
        'roles' => $out,
        'policy' => $policy,
        'global_disable_file_edit' => (bool) get_option(WPSC_OPT_DISABLE_FILE_EDIT, false),
    ]);
}

function wpsc_am_apply($req) {
    $p = $req->get_json_params();
    $policy = (isset($p['policy']) && is_array($p['policy'])) ? $p['policy'] : [];
    $global_disable = !empty($p['global_disable_file_edit']);

    // Sanitize: only known feature keys + known levels
    $clean = [];
    foreach ($policy as $role_slug => $features) {
        if (!is_string($role_slug) || !is_array($features)) continue;
        $clean[sanitize_key($role_slug)] = wpsc_am_sanitize_features($features);
    }
    update_option(WPSC_OPT_ACCESS_POLICY, $clean);
    update_option(WPSC_OPT_DISABLE_FILE_EDIT, $global_disable);

    $applied = [];
    foreach ($clean as $role_slug => $features) {
        $role = get_role($role_slug);
        if (!$role) continue;
        $caps_to_set = wpsc_am_features_to_caps($features);
...
function wpsc_am_revert($req) {
    ...
    delete_option(WPSC_OPT_ACCESS_POLICY);
    return rest_ensure_response(['ok' => true]);
}

function wpsc_am_sanitize_features($f) {
...
function wpsc_am_features_to_caps($features) {
    $caps = [];
    ...
    if (isset($features['snippet_management'])) {
        $caps['manage_snippets'] = $features['snippet_management'] === 'granted';
    }
    if (isset($features['file_editor'])) {
        $caps['edit_files'] = $features['file_editor'] === 'granted';
    }
    if (isset($features['theme_editor'])) {
        $caps['edit_themes'] = $features['theme_editor'] === 'granted';
    }
    if (isset($features['plugin_editor'])) {
        $caps['edit_plugins'] = $features['plugin_editor'] === 'granted';
    }
    if (isset($features['pixels'])) {
        $caps['manage_pixels'] = $features['pixels'] === 'granted';
    }
    if (isset($features['access_management'])) {
        $caps['manage_access'] = $features['access_management'] === 'granted';
    }
    return $caps;
}
...
function wpsc_am_user_has_cap_filter($allcaps, $caps, $args, $user) {
    ...
    if ($policy_cache === null) {
        $policy_cache = get_option(WPSC_OPT_ACCESS_POLICY, []);
        if (!is_array($policy_cache)) $policy_cache = [];
    }
    if ($global_disable === null) {
        $global_disable = (bool) get_option(WPSC_OPT_DISABLE_FILE_EDIT, false);
    }
    if ($global_disable) {
    ...
    foreach ($user->roles as $role_slug) {
        ...
        foreach (wpsc_am_denied_caps($features) as $cap) {
...
function wpsc_am_denied_caps($features) {
    $d = [];
