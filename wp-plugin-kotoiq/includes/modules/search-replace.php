...
add_action('rest_api_init', function () {
    ...
    kotoiq_register_rest_route('/search-replace/tables', [
        'methods'  => 'POST',
        'callback' => 'kotoiq_sr_list_tables',
        'permission_callback' => 'kotoiq_perm_read',
    ]);
    kotoiq_register_rest_route('/search-replace/scan', [
        'methods'  => 'POST',
        'callback' => 'kotoiq_sr_scan',
        'permission_callback' => 'kotoiq_perm_write',
    ]);
    kotoiq_register_rest_route('/search-replace/restore', [
        'methods'  => 'POST',
        'callback' => 'kotoiq_sr_restore',
        'permission_callback' => 'kotoiq_perm_write',
    ]);
});

function kotoiq_sr_list_tables($req) {
    global $wpdb;
    $prefix = $wpdb->prefix;
    ...
    // restrict for the WP DB user (LiteSpeed-backed hosts trip on this).
...
function kotoiq_sr_scan($req) {
    ...
    foreach ($rows as $row) {
        ...
        foreach ($cols as $col) {
            ...
            $new = kotoiq_sr_replace_value($orig, $search, $replace, $options);
            if ($new === $orig) continue;

            $count_in_field = kotoiq_sr_count_matches($orig, $search, $options);
            ...
            if (count($sample) < $sample_cap) {
                $sample[] = [
                    'pk'     => $pk_val,
                    'column' => $col,
                    'before' => kotoiq_sr_truncate($orig, 400),
                    'after'  => kotoiq_sr_truncate($new, 400),
                ];
            }
            if (!$dry_run) {
...
function kotoiq_sr_restore($req) {
    global $wpdb;
    $p = $req->get_json_params();
...
/**
 * Serialized-safe recursive replacement. PHP serialize() stores byte
 * lengths in s:N:"..." headers; a naive str_replace shrinks/grows the body
 * but leaves N wrong → corruption. We unserialize first, walk, re-serialize.
 */
function kotoiq_sr_replace_value($value, $from, $to, $options) {
    if (is_string($value) && is_serialized($value)) {
        $unser = @unserialize($value, ['allowed_classes' => false]);
        if ($unser !== false || $value === 'b:0;') {
            return serialize(kotoiq_sr_replace_value($unser, $from, $to, $options));
        }
    }
    if (is_array($value)) {
        $out = [];
        foreach ($value as $k => $v) {
            $newK = is_string($k) ? kotoiq_sr_replace_string($k, $from, $to, $options) : $k;
            $out[$newK] = kotoiq_sr_replace_value($v, $from, $to, $options);
        }
        return $out;
    }
    if (is_object($value)) return $value;
    if (is_string($value)) return kotoiq_sr_replace_string($value, $from, $to, $options);
    return $value;
}

function kotoiq_sr_replace_string($str, $from, $to, $options) {
...
function kotoiq_sr_count_matches($value, $needle, $options) {
...
function kotoiq_sr_truncate($value, $max = 400) {
